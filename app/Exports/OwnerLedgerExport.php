...
class OwnerLedgerExport implements
    FromCollection,
    WithHeadings,
    WithStyles,
    WithColumnWidths,
    ...
    ShouldAutoSize
{
    ...
    public function collection(): Collection
    {
        return $this->entries->map(fn($e) => [
            ...
            'Notes' => $e['notes'] ?? '—',
            'Debit (Payout)' => number_format($e['debit'], 2),
            'Credit (Deposit)' => number_format($e['credit'], 2),
    ...
    public function styles(Worksheet $sheet): array
    {
        ...
        $sheet->setCellValue("A" . ($summaryRow + 1), 'Total Payouts (Rs.)');
        $sheet->setCellValue("B" . ($summaryRow + 1), number_format($this->summary['total_debit'], 2));
        $sheet->setCellValue("A" . ($summaryRow + 2), 'Total Deposits (Rs.)');
