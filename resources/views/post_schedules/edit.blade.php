...
@section('content')
    <div class="space-y-6">
        <x-common.page-breadcrumb pageTitle="Edit Post Schedule Task" />
        ...
        <div class="max-w-4xl mx-auto rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03] sm:p-8">
            <div class="flex items-center justify-between pb-4 mb-6 border-b border-gray-100 dark:border-gray-800">
                <div>
                    <h3 class="text-lg font-black text-gray-900 dark:text-white/90 uppercase tracking-tight">
                        Edit Post Schedule Duty
                    </h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        Update task details, shift timings, or assigned staff
                    </p>
                </div>
                <a href="{{ route('post-schedules.index', ['day' => $postSchedule->day_of_week]) }}"
            ...
            <form action="{{ route('post-schedules.update', $postSchedule) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select name="day_of_week" required
                            ...
                            @foreach($days as $k => $d)
                                <option value="{{ $k }}" {{ old('day_of_week', $postSchedule->day_of_week) == $k ? 'selected' : '' }}>
                                    {{ $d }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="post_schedule_head_id" required
                            ...
                            @foreach($heads as $head)
                                <option value="{{ $head->id }}" {{ old('post_schedule_head_id', $postSchedule->post_schedule_head_id) == $head->id ? 'selected' : '' }}>
                                    {{ $head->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="employee_name" value="{{ old('employee_name', $postSchedule->employee_name) }}" required
                    ...
                    <div>
                        ...
                        <select name="user_id"
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('user_id', $postSchedule->user_id) == $user->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="location" value="{{ old('location', $postSchedule->location) }}"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="time" name="start_time" value="{{ old('start_time', $postSchedule->start_time ? \Carbon\Carbon::parse($postSchedule->start_time)->format('H:i') : '') }}"
                                class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3.5 py-2.5 text-sm text-gray-900 dark:text-white focus:border-brand-500 focus:outline-none">
                        ...
                        <div>
                            ...
                            <input type="time" name="end_time" value="{{ old('end_time', $postSchedule->end_time ? \Carbon\Carbon::parse($postSchedule->end_time)->format('H:i') : '') }}"
                                class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3.5 py-2.5 text-sm text-gray-900 dark:text-white focus:border-brand-500 focus:outline-none">
                ...
                <div>
                    ...
                    <input type="text" name="task_title" value="{{ old('task_title', $postSchedule->task_title) }}" required
                ...
                <div>
                    ...
                    <textarea name="duties" rows="4"
                        ...
                        class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3.5 py-2.5 text-sm text-gray-900 dark:text-white focus:border-brand-500 focus:outline-none">{{ old('duties', $postSchedule->duties) }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="notes" value="{{ old('notes', $postSchedule->notes) }}"
                    ...
                    <div class="flex items-center gap-6 pt-2">
                        <div>
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', $postSchedule->sort_order) }}" min="0"
                        ...
                        <div class="pt-5">
                            <label class="flex items-center gap-2 cursor-pointer text-sm font-bold text-gray-800 dark:text-gray-200">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $postSchedule->is_active) ? 'checked' : '' }}
