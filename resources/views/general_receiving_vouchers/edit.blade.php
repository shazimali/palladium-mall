...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit General Receiving Voucher — {{ $voucher->voucher_no }}" />

    <form action="{{ route('general-receiving-vouchers.update', $voucher) }}" method="POST"
        @submit.prevent="handleSubmit($event)"
        x-data="{
            receivedFromType: '{{ old('received_from_type', $voucher->received_from_type) }}',
            ...
            isNotesManuallyEdited: {{ (old('notes', $voucher->notes ?? '') && !str_starts_with(old('notes', $voucher->notes ?? ''), 'Payment received')) ? 'true' : 'false' }},
            ...
            handleSubmit(event) {
                ...
                Swal.fire({
                    title: 'Confirm Update General Receiving Voucher',
                    text: 'Are you sure you want to update and print this general receiving voucher for Rs. ' + amt.toLocaleString('en-US', {minimumFractionDigits: 2}) + '?',
                    icon: 'question',
