...
<head>
    ...
    <title>Print General Receiving Vouchers Report - PALLADIUM MALL</title>
    <!-- Tailwind CSS CDN -->
    ...
    <div class="max-w-6xl w-full mx-auto printable-container rounded-3xl bg-white p-6 sm:p-10 border border-gray-300 shadow-xl text-gray-900 font-sans">
        ...
        <div class="text-center mb-6 pb-4 border-b border-gray-300">
            ...
            <h2 class="text-xl sm:text-2xl font-black tracking-tight text-blue-700 uppercase">
                General Receiving Vouchers Report
            </h2>
        </div>
        ...
        <div class="mb-6 rounded-2xl bg-gray-50 border border-gray-300 p-4 font-sans text-xs">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-gray-800 font-semibold">
                ...
                <div>
                    <span class="text-gray-500 block text-[10px] uppercase font-bold">Party / Landlord:</span>
                    <span class="font-bold text-gray-900">
                        {{ $selectedParty ?? ($selectedLandlord ?? 'All Payees') }}
                    </span>
                </div>
                <div>
                    <span class="text-gray-500 block text-[10px] uppercase font-bold">Deposit Account:</span>
                    <span class="font-bold text-gray-900">{{ $selectedAccount ?? 'All Deposit Accounts' }}</span>
                </div>
                <div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-200 mb-6 text-sm font-bold">
            ...
            <div>
                ...
                <span class="text-emerald-700 font-black ml-1 text-base">Rs. {{ number_format($totalAmount, 2) }}</span>
            </div>
            <div>
                    ...
                    @forelse($vouchers as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-2.5 px-3 font-bold text-gray-900">
                                ...
                                @elseif($item->received_from_type === 'landlord')
                                    {{ $item->landlord->name ?? '—' }} <span class="text-[10px] text-gray-500 font-normal">(Landlord)</span>
                                @elseif($item->received_from_type === 'account')
                                    {{ $item->fromPaymentAccount->name ?? '—' }} <span class="text-[10px] text-gray-500 font-normal">(Account Transfer)</span>
                                @else
                                    —
                                @endif
                            </td>
                            ...
                            <td class="py-2.5 px-3 text-right font-black text-emerald-700 font-mono text-sm whitespace-nowrap">
