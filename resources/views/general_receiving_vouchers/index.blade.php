...
@section('content')
    ...
    <div
        x-data="{ showTable: {{ (request()->anyFilled(['search', 'party_id', 'landlord_id', 'payment_account_id', 'start_date', 'end_date']) || request()->has('show_search')) ? 'true' : 'false' }} }">
        <x-common.component-card title="" desc="">

            {{-- Top bar --}}
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-center">
                <div class="flex flex-wrap items-center gap-3">
                    {{-- 1. New General Receipt Button --}}
                    ...
                    {{-- 2. Search Button --}}
                    @if(auth()->user()->hasPermission('general_receiving_vouchers.search') || auth()->user()->hasPermission('general_receiving_vouchers.view') || auth()->user()->isSuperAdmin())
                    ...
                    <form action="{{ route('general-receiving-vouchers.index') }}" method="GET"
                        ...
                        <div class="relative col-span-1 lg:col-span-2">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Search voucher #, manual #, landlord, party..."
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td class="px-4 py-3 font-semibold">
                                        @if($voucher->received_from_type === 'account')
                                            <span class="text-blue-600 dark:text-blue-400">🏦 Transfer from
                                                {{ $voucher->fromPaymentAccount->name ?? 'Account' }}</span>
                                        @elseif($voucher->received_from_type === 'landlord' || $voucher->landlord_id)
                                            <span class="text-amber-600 dark:text-amber-400">🏠 Landlord:
                                                {{ $voucher->landlord ? $voucher->landlord->name : 'N/A' }}</span>
                                        @else
                                    ...
                                    <td class="px-4 py-3 text-xs">
                                        ...
                                        <span
                                            class="ml-1 text-[10px] text-gray-400 uppercase">({{ $voucher->payment_method }})</span>
                                    </td>
