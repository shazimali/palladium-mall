...
@section('content')
    ...
    <x-common.component-card title="Statement Filters" desc="Select date range to analyze profit & loss distribution">
        <form method="GET" action="{{ route('reports.profit-loss') }}">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 max-w-4xl">
                {{-- Date From --}}
                <div>
                ...
                <div class="flex items-end gap-3">
                    <button type="submit"
                        class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-5 py-2.5 text-sm font-medium text-white hover:bg-brand-600 transition-colors w-full sm:w-auto">
    ...
    <div class="my-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white dark:bg-white/[0.02] border border-gray-250 dark:border-gray-800 p-4 rounded-xl">
        <div>
            <h3 class="text-sm font-semibold text-gray-800 dark:text-white">Palladium Mall Statement Period</h3>
            <p class="text-xs text-gray-500 mt-0.5">Showing accounts breakdown from <strong class="text-gray-700 dark:text-gray-350">{{ Carbon\Carbon::parse($filters['date_from'])->format('d M Y') }}</strong> to <strong class="text-gray-700 dark:text-gray-350">{{ Carbon\Carbon::parse($filters['date_to'])->format('d M Y') }}</strong></p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        ...
                        @foreach($incomeBreakdown as $type => $amount)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    {{ $type === 'rent' ? '🏠 Rent Collected' : ($type === 'maintenance' ? '🛠️ Maintenance Charges' : ($type === 'fine' ? '⚠️ Fines Collected' : '⚡ Utility: ' . ucfirst($type))) }}
                                </td>
        ...
        <x-common.component-card title="Expense Breakdown" desc="Summary of expenses grouped by head">
            @if(empty($expensesByHead))
                <div class="py-12 text-center text-gray-400 dark:text-gray-600 border border-dashed border-gray-200 dark:border-gray-800 rounded-xl">
    ...
    <div class="mt-6">
        <x-common.component-card title="Managing Partners Earnings Distribution" desc="Splitting net profit/loss based on partner profiles percentage shares">
            
            @if(abs($totalOwnerSharePct - 100.00) > 0.01)
                <div class="mb-4 rounded-xl bg-orange-50 border border-orange-200 p-4 text-xs text-orange-700 dark:bg-orange-950/10 dark:border-orange-850 dark:text-orange-400">
                    ⚠️ The active owners partnership percentages sum is <strong>{{ number_format($totalOwnerSharePct, 2) }}%</strong>, not 100.00%. Correct calculations require partnership shares to distribute fully to 100%. Adjust shares in <a href="{{ route('owners.index') }}" class="underline font-bold">Owners Profile settings</a>.
                </div>
            @endif

            @if(empty($distribution))
                <div class="py-12 text-center text-gray-400 dark:text-gray-600 border-2 border-dashed border-gray-250 dark:border-gray-800 rounded-xl bg-white dark:bg-gray-900">
                    <p class="text-xs">No partners registered. Add mall partners in <a href="{{ route('owners.index') }}" class="text-brand-500 hover:underline">Owners Profile</a> to calculate profit splits.</p>
                </div>
            @else
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @foreach($distribution as $row)
                                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                                    ...
                                    <td class="px-4 py-3 text-right font-bold {{ $row['share'] >= 0 ? 'text-green-600' : 'text-red-500' }}">
