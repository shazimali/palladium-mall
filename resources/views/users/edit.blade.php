    ...
    <div class="mx-auto w-full max-w-[750px]">
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 pb-4 mb-6">
                <div>
                    <h3 class="text-base font-extrabold text-gray-900 dark:text-white">Edit User: {{ $user->name }}</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Update account credentials, roles, and employee settings.</p>
                </div>
                <div class="flex items-center gap-3">
                    <a href="{{ route('users.show', $user) }}" class="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline">
                        👁️ View Profile
                    </a>
                    <span class="text-gray-300 dark:text-gray-700">|</span>
                    <a href="{{ route('users.index') }}" class="text-xs font-bold text-gray-500 dark:text-gray-400 hover:underline">
                        ← Back to Users
                    </a>
                </div>
            </div>
            ...
            <form action="{{ route('users.update', $user) }}" method="POST"
                x-data="{ 
                    isEmployee: {{ old('is_employee', $user->is_employee ? '1' : '0') == '1' ? 'true' : 'false' }},
                    showPassword: false,
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" placeholder="e.g., Jane Doe"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" placeholder="e.g., jane@example.com"
                    ...
                    <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-gray-900/30">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <span class="mt-1 block text-[11px] text-gray-400">Leave blank to keep current password.</span>
                                @error('password')
                                    <p class="mt-1 text-xs text-red-500 font-semibold">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-1">
                                    ...
                                    <template x-if="!password && !passwordConfirmation">
                                        <span class="text-[11px] text-gray-400">Confirmation comparison.</span>
                                    </template>
                                </div>
                    ...
                    <div>
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" 
                                ...
                                class="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500/30 dark:border-gray-700 dark:bg-gray-900 {{ $user->id === auth()->id() ? 'opacity-50 cursor-not-allowed' : '' }}" />
                            <span class="text-xs font-bold text-gray-700 dark:text-gray-300 {{ $user->id === auth()->id() ? 'opacity-50' : '' }}">Account is Active</span>
                        </label>
                        @if($user->id === auth()->id())
                            <span class="mt-1 block text-xs text-yellow-600 dark:text-yellow-400">You cannot deactivate your own logged-in account.</span>
                        @endif
                    </div>
                </div>
                    ...
                    <div x-show="isEmployee" x-cloak
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="employee_code" value="{{ old('employee_code', $profile?->employee_code) }}" placeholder="e.g. EMP-001"
                            ...
                            <div>
                                ...
                                <input type="text" name="designation" value="{{ old('designation', $profile?->designation) }}" placeholder="e.g. Operations Officer"
                            ...
                            <div>
                                ...
                                <input type="text" name="department" value="{{ old('department', $profile?->department) }}" placeholder="e.g. Maintenance / Security"
                                    class="w-full h-10 px-3 text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="joined_at" value="{{ old('joined_at', $profile?->joined_at?->toDateString()) }}"
                                    class="w-full h-10 px-3 text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-mono">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="basic_salary" value="{{ old('basic_salary', $profile?->basic_salary ?? 0) }}" placeholder="0.00"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="fuel_allowance" value="{{ old('fuel_allowance', $profile?->fuel_allowance ?? 0) }}"
                                    class="w-full h-10 px-3 text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-mono">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="attendance_incentive" value="{{ old('attendance_incentive', $profile?->attendance_incentive ?? 0) }}"
                                    class="w-full h-10 px-3 text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-mono">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" max="100" name="collection_incentive_pct" value="{{ old('collection_incentive_pct', $profile?->collection_incentive_pct ?? 0) }}"
                                    class="w-full h-10 px-3 text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-mono">
                            </div>
                        </div>
