                ...
                <form :action="'{{ url('jv-vouchers') }}/' + payVoucherId + '/pay'" method="POST" class="mt-5 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="w-full relative" @click.away="openPayAccount = false; highlightedPayAccountIndex = -1">
                            ...
                            <button type="button" id="jv-pay-account-btn"
                                ...
                                :class="document.getElementById('jv-pay-account-error')?.style.display !== 'none' && !payAccountId ? 'border-red-500' : ''">
                        ...
                        <p id="jv-pay-account-error" style="display:none;" class="mt-1.5 text-xs font-semibold text-red-600 flex items-center gap-1">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
                        ...
                        <button type="button" id="jv-confirm-settlement-btn"
                            onclick="confirmJvSettlement(this)"
...
@endif
function confirmJvSettlement(btn) {
    const form = btn.closest('form');
    const payAccountInput = form.querySelector('input[name="payment_account_id"]');
    const errorEl = document.getElementById('jv-pay-account-error');
    const dropdownBtn = document.getElementById('jv-pay-account-btn');
    const voucherNo = document.querySelector('[x-text="payVoucherNo"]')?.textContent?.trim() || '';
    const amount = document.querySelector('[x-text="\'Rs. \' + payAmount"]')?.textContent?.trim() || '';
    ...
    Swal.fire({
        ...
        html: `<div style="text-align:left;font-size:15px;">
            <p style="margin-bottom:10px;">You are about to mark this JV Voucher as <strong>Paid/Settled</strong>.</p>
            <table style="width:100%;border-collapse:collapse;">
                <tr><td style="padding:6px 0;color:#6b7280;font-weight:600;">Voucher No:</td><td style="padding:6px 0;font-weight:900;color:#6d28d9;font-family:monospace;">${voucherNo}</td></tr>
                <tr><td style="padding:6px 0;color:#6b7280;font-weight:600;">Amount:</td><td style="padding:6px 0;font-weight:900;color:#059669;font-family:monospace;">${amount}</td></tr>
            </table>
