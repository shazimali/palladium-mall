...
@section('content')
    <x-common.page-breadcrumb pageTitle="New JV Voucher" />
    ...
    <form action="{{ route('jv-vouchers.store') }}" method="POST" enctype="multipart/form-data"
        ...
        x-data="{
            ...
            handleSubmit(event) {
                ...
                Swal.fire({
                    title: 'Confirm Create JV Voucher',
                    text: 'Are you sure you want to save and generate this Journal Voucher for Rs. ' + amt.toLocaleString('en-US', {minimumFractionDigits: 2}) + '?',
                    icon: 'question',
        ...
        <div class="mx-auto max-w-4xl bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 p-6 sm:p-8 shadow-sm text-gray-900 dark:text-white font-sans relative">
            ...
            <div class="flex flex-col gap-[2px] bg-gray-200 dark:bg-gray-700 rounded-2xl overflow-visible mb-6 border border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[2px] bg-gray-200 dark:bg-gray-700 relative z-40 rounded-t-2xl">
                    ...
                    <div class="grid grid-cols-3 min-h-[52px]">
                        <div class="bg-brand-600 dark:bg-brand-900 text-white px-4 py-3 flex items-center font-extrabold text-sm sm:text-base tracking-wide">Category <span class="text-rose-300 ml-1">*</span></div>
                        <div class="col-span-2 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 flex items-center md:rounded-tr-2xl">
                            <div class="w-full relative" @click.away="openCategory = false; highlightedCategoryIndex = -1">
                                ...
                                <button type="button" @click="openCategory = !openCategory; if(openCategory) { $nextTick(() => $refs.categorySearchInput.focus()) }"
                                    ...
                                    <span x-text="selectedCategoryName ? selectedCategoryName : 'Select Category...'" class="truncate"></span>
                                ...
                                <div x-show="openCategory" x-transition x-cloak
                                    ...
                                    <div class="p-3 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
                                        <input type="text" x-ref="categorySearchInput" x-model="searchCategory" placeholder="Type category name to search..."
                                    ...
                                    <div class="max-h-[300px] overflow-y-auto p-1.5 space-y-1 text-sm">
                                        ...
                                        <div x-show="filteredCategories.length === 0" class="px-4 py-3 text-center text-xs font-semibold text-gray-400">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[2px] bg-gray-200 dark:bg-gray-700 relative z-30">
                    ...
                    <div class="grid grid-cols-3 min-h-[52px]">
                        <div class="bg-brand-600 dark:bg-brand-900 text-white px-4 py-3 flex items-center font-extrabold text-sm sm:text-base tracking-wide">Payment Amount <span class="text-rose-300 ml-1">*</span></div>
                        <div class="col-span-2 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 flex items-center">
                    ...
                    <div class="grid grid-cols-3 min-h-[52px]">
                        <div class="bg-brand-600 dark:bg-brand-900 text-white px-4 py-3 flex items-center font-extrabold text-sm sm:text-base tracking-wide">Voucher Status <span class="text-rose-300 ml-1">*</span></div>
                        <div class="col-span-2 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 flex items-center gap-4">
                            <label class="flex items-center gap-2 cursor-pointer font-bold text-xs sm:text-sm">
                                <input type="radio" name="status" value="unpaid" x-model="status" class="text-amber-600 focus:ring-amber-500">
                                <span :class="status === 'unpaid' ? 'text-amber-600 dark:text-amber-400 font-black' : 'text-gray-700 dark:text-gray-300'">Unpaid (Accrued)</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer font-bold text-xs sm:text-sm">
                                <input type="radio" name="status" value="paid" x-model="status" class="text-green-600 focus:ring-green-500">
                                <span :class="status === 'paid' ? 'text-green-600 dark:text-green-400 font-black' : 'text-gray-700 dark:text-gray-300'">Paid (Settled)</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="status === 'paid'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-[2px] bg-gray-200 dark:bg-gray-700 relative z-20">
                    ...
                    <div class="grid grid-cols-3 min-h-[52px]">
                        <div class="bg-brand-600 dark:bg-brand-900 text-white px-4 py-3 flex items-center font-extrabold text-sm sm:text-base tracking-wide">Method & Date <span class="text-rose-300 ml-1">*</span></div>
                        <div class="col-span-2 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 flex items-center gap-2">
                            ...
                            <input type="date" name="paid_date" value="{{ old('paid_date', date('Y-m-d')) }}" :required="status === 'paid'"
                                class="w-1/2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl px-2 py-2 text-xs sm:text-sm font-bold text-gray-900 dark:text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500 focus:outline-none">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[2px] bg-gray-200 dark:bg-gray-700 relative z-10 rounded-b-2xl">
                    ...
                    <div class="grid grid-cols-3 min-h-[52px]">
                        <div class="bg-brand-600 dark:bg-brand-900 text-white px-4 py-3 flex items-center font-extrabold text-sm sm:text-base tracking-wide rounded-bl-2xl">Reference #</div>
                        <div class="col-span-2 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 flex items-center">
                            <input type="text" name="reference" value="{{ old('reference') }}" placeholder="Optional bill / ref #"
                    ...
                    <div class="grid grid-cols-3 min-h-[52px]">
                        <div class="bg-brand-600 dark:bg-brand-900 text-white px-4 py-3 flex items-center font-extrabold text-sm sm:text-base tracking-wide">Attach Receipt</div>
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-800">
                <a href="{{ route('jv-vouchers.index') }}" class="px-5 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold transition-all text-xs sm:text-sm">
                    Cancel
                </a>
