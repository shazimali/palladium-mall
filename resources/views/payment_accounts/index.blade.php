    ...
    <x-common.component-card title="" desc="">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                @if(auth()->user()->hasPermission('payment_accounts.create') || auth()->user()->isSuperAdmin())
                    <a href="{{ route('payment-accounts.create') }}"
        ...
        <div
            ...
            <form action="{{ route('payment-accounts.index') }}" method="GET" class="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($paymentAccounts as $index => $account)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-4 py-3 text-gray-400">{{ $paymentAccounts->firstItem() + $index }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if(auth()->user()->hasPermission('payment_accounts.edit') || auth()->user()->isSuperAdmin())
                                        <a href="{{ route('payment-accounts.edit', $account) }}"
                        ...
                        <tr>
                            <td colspan="8" class="px-4 py-12 text-center text-gray-400 dark:text-gray-600">
                                <svg class="mx-auto mb-3 h-10 w-10 opacity-40" fill="none" stroke="currentColor"
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M12 6v12m-3-2.818.879.596M12 6M12 6a3.5 3.5 0 1 0 0 7c.879 0 1.62-.318 2.121-.818M12 6a3.5 3.5 0 0 1 3.5 3.5c0 .879-.318 1.62-.818 2.121M12 13m0 0v5m0-5a3.5 3.5 0 0 1-3.5-3.5c0-.879.318-1.62.818-2.121M12 13a3.5 3.5 0 0 0 3.5 3.5c.879 0 1.62-.318 2.121-.818" />
                                </svg>
                                No payment accounts found. <a href="{{ route('payment-accounts.create') }}" class="text-brand-500 hover:underline">Add one.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
