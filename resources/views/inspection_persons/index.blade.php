...
@section('content')
    <x-common.page-breadcrumb pageTitle="Inspection Persons" />
    ...
    <x-common.component-card title="Inspection Persons / Inspectors" desc="Manage inspection team profiles and active inspectors">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                @if(auth()->user()->hasPermission('inspection_persons.create') || auth()->user()->isSuperAdmin())
                    <a href="{{ route('inspection-persons.create') }}"
        ...
        <div
            ...
            <form action="{{ route('inspection-persons.index') }}" method="GET" class="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <div class="relative flex-1 max-w-md">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search inspectors..."
        ...
        <div class="overflow-hidden border border-gray-200 rounded-xl dark:border-gray-800">
            <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($inspectionPersons as $index => $person)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-4 py-3 text-gray-400">{{ $inspectionPersons->firstItem() + $index }}</td>
                            <td class="px-4 py-3 font-semibold text-gray-800 dark:text-white/90">
                                {{ $person->name }}
                            </td>
                            <td class="px-4 py-3">{{ $person->designation ?? '—' }}</td>
                            <td class="px-4 py-3">{{ $person->phone ?? '—' }}</td>
                            <td class="px-4 py-3">{{ $person->email ?? '—' }}</td>
                            <td class="px-4 py-3">
                                @if($person->is_active)
                                    <span class="inline-flex items-center rounded-md bg-green-50 px-2.5 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/20 dark:text-green-400">
                                        Active
                                    </span>
                                @else
                                    <span class="inline-flex items-center rounded-md bg-red-50 px-2.5 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900/20 dark:text-red-400">
                                        Inactive
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    {{-- Edit --}}
                                    @if(auth()->user()->hasPermission('inspection_persons.edit') || auth()->user()->isSuperAdmin())
                                        <a href="{{ route('inspection-persons.edit', $person) }}"
                                    ...
                                    {{-- Delete --}}
                                    @if(auth()->user()->hasPermission('inspection_persons.delete') || auth()->user()->isSuperAdmin())
                                        <form action="{{ route('inspection-persons.destroy', $person) }}" method="POST" x-data
                                            @submit.prevent="if(confirm('Remove inspection member {{ $person->name }}? Past inspection records will retain their text name but lose this profile association.')) $el.submit()">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-4 py-12 text-center text-gray-400 dark:text-gray-600">
                                ...
                                No inspection persons registered. <a href="{{ route('inspection-persons.create') }}" class="text-brand-500 hover:underline">Add one.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($inspectionPersons->hasPages())
            <div class="border-t border-gray-100 p-4 dark:border-gray-800">
                {{ $inspectionPersons->links() }}
            </div>
        @endif
