...
@section('content')
    <x-common.page-breadcrumb pageTitle="Expense Heads" />
    ...
    <x-common.component-card title="Expense Heads / Categories" desc="Manage accounting heads for registering business and operational expenses">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                @if(auth()->user()->hasPermission('expense_heads.create') || auth()->user()->isSuperAdmin())
                    <a href="{{ route('expense-heads.create') }}"
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <form action="{{ route('expense-heads.index') }}" method="GET" class="flex flex-col gap-4 sm:flex-row sm:items-center">
                <!-- Search Input -->
                <div class="relative flex-1 max-w-md">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search expense heads..."
        ...
        <div class="overflow-hidden border border-gray-200 rounded-xl dark:border-gray-800">
            <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($expenseHeads as $index => $head)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-4 py-3 text-gray-400">{{ $expenseHeads->firstItem() + $index }}</td>
                            <td class="px-4 py-3 font-semibold text-gray-800 dark:text-white/90">
                                {{ $head->name }}
                            </td>
                            <td class="px-4 py-3">{{ $head->description ?? '—' }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    {{-- Edit --}}
                                    @if(auth()->user()->hasPermission('expense_heads.edit') || auth()->user()->isSuperAdmin())
                                        <a href="{{ route('expense-heads.edit', $head) }}"
                                    ...
                                    {{-- Delete --}}
                                    @if(auth()->user()->hasPermission('expense_heads.delete') || auth()->user()->isSuperAdmin())
                                        <form action="{{ route('expense-heads.destroy', $head) }}" method="POST" x-data
                                            @submit.prevent="if(confirm('Are you sure you want to delete this Expense Head? This cannot be undone.')) $el.submit()">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-12 text-center text-gray-400 dark:text-gray-600">
                                ...
                                No expense heads registered. <a href="{{ route('expense-heads.create') }}" class="text-brand-500 hover:underline">Add one.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
