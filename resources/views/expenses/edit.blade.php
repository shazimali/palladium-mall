...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-6">
        <div class="mb-6 flex items-center gap-2 text-sm font-semibold text-gray-500 dark:text-gray-400">
            ...
            <span>/</span>
            <span class="text-gray-800 dark:text-white/90">Edit Recorded Expense: {{ $expense->voucher_no }}</span>
        </div>

        <x-common.component-card :title="'Edit Recorded Expense: ' . $expense->voucher_no" desc="Modify details of the logged expense transaction">
                        ...
                        <div class="flex items-center gap-4 min-w-0">
                            ...
                            <div class="min-w-0">
                                <p class="text-xs font-extrabold uppercase tracking-wider text-amber-600 dark:text-amber-400">
                                    Edit Expense Voucher: {{ $expense->voucher_no }}
                                </p>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <template x-if="adjustedBalance !== null">
                            <div class="mt-2 text-sm font-bold text-gray-700 dark:text-gray-300 bg-amber-50 dark:bg-amber-950/20 p-3.5 rounded-xl border border-amber-200 dark:border-amber-800/40 flex justify-between items-center">
                                ...
                                <span :class="parseFloat(adjustedBalance) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'" class="font-black text-lg font-mono" x-text="'Rs. ' + Number(adjustedBalance).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span>
                            </div>
                        </template>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="notes" placeholder="Enter specific transaction details here..." rows="3"
                                  class="{{ $input }} font-medium text-base">{{ old('notes', $expense->notes) }}</textarea>
