...
<div x-data="{
    ...
    <div class="rounded-2xl border-2 border-gray-200 bg-gray-50/70 p-6 dark:border-gray-800 dark:bg-white/[0.02]">
        ...
        <div>
            <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">
                Landlord / Owner <span class="text-xs font-semibold text-gray-400 dark:text-gray-500">(Optional)</span>
            </label>
    ...
    <div class="rounded-2xl border-2 border-gray-200 bg-gray-50/70 p-6 dark:border-gray-800 dark:bg-white/[0.02]">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 max-w-3xl">
            ...
            <div>
                ...
                <div class="relative">
                    <span class="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-base font-black text-gray-400">Rs.</span>
                    <input type="hidden" name="default_monthly_rent" :value="rentRaw">
                    <input type="text"
                        :value="formatNumber(rentRaw)"
                        @input="onInput('rentRaw', $event)"
                        placeholder="0.00"
                        class="w-full rounded-2xl border-2 border-gray-300 bg-white pl-12 pr-5 py-3.5 text-lg font-bold text-gray-900 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                </div>
    ...
    {{-- Nominee Section --}}
    <div class="rounded-2xl border-2 border-blue-200 bg-blue-50/40 p-6 dark:border-blue-900/40 dark:bg-white/[0.01]">
        <h4 class="mb-5 text-base sm:text-lg font-black uppercase tracking-wider text-blue-700 dark:text-blue-400">
            Nominee Details
        </h4>
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
            {{-- Nominee Name --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Nominee Name</label>
            ...
            {{-- Relation Type --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Relation</label>
            ...
            {{-- Relation Name --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Of (Father/Husband Name)</label>
    ...
    <div class="rounded-2xl border-2 border-green-200 bg-green-50/40 p-6 dark:border-green-900/40 dark:bg-white/[0.01]">
        <h4 class="mb-5 text-base sm:text-lg font-black uppercase tracking-wider text-green-700 dark:text-green-400">
            Financial Summary
        </h4>
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {{-- Total Amount --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Total Amount</label>
            ...
            {{-- Received Amount --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Received Amount</label>
            ...
            {{-- Credit / Balance --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Credit / Balance</label>
                <div class="flex items-center gap-2 rounded-2xl border-2 border-gray-200 bg-gray-100 px-5 py-3.5 dark:border-gray-800 dark:bg-gray-950">
                    <span class="text-base text-gray-400">🔒</span>
                    <span class="text-lg font-black font-mono text-red-600 dark:text-red-400" x-text="'Rs. ' + credit"></span>
                </div>
            </div>

            {{-- Received From --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Received From</label>
    ...
    <div class="rounded-2xl border-2 border-amber-200 bg-amber-50/40 p-6 dark:border-amber-900/40 dark:bg-white/[0.01]">
        <h4 class="mb-5 text-base sm:text-lg font-black uppercase tracking-wider text-amber-700 dark:text-amber-400">
            Office Record
        </h4>
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {{-- File No. --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">File No.</label>
            ...
            {{-- Approved By --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Approved By</label>
            ...
            {{-- Received By --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Received By</label>
            ...
            {{-- Approved Date --}}
            <div>
                <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Approved Date</label>
        ...
        {{-- Notes / Remarks --}}
        <div class="mt-6">
            <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Billing Notes / Remarks</label>
            <textarea name="notes" rows="3"
                placeholder="Remarks, billing logs or structural information about this unit..."
                class="w-full rounded-2xl border-2 border-gray-300 bg-white px-5 py-3.5 text-lg font-semibold text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white">{{ old('notes', $unit->notes) }}</textarea>
            @error('notes')
                <p class="mt-1.5 text-sm font-semibold text-red-500">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border-2 transition-all duration-300"
        ...
        <div class="flex items-center gap-4 px-6 py-5">
            {{-- Icon --}}
            <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-gray-100 dark:bg-gray-700 text-2xl">
                🏢
            </div>

            {{-- Label --}}
            <div class="flex-1 min-w-0">
                <p class="text-base font-black leading-tight text-blue-700 dark:text-blue-400">
                    Other-Owned Unit
                </p>
                <p class="mt-1 text-sm font-bold leading-snug"
                    :class="isSelf ? 'text-blue-600 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'">
            ...
            <button type="button"
                ...
                class="relative inline-flex h-7 w-14 flex-shrink-0 cursor-pointer rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:ring-offset-2 dark:focus:ring-offset-gray-900"
                role="switch"
                :aria-checked="isSelf.toString()">
                <span
                    :style="'transform: translateX(' + (isSelf ? '28px' : '2px') + ') translateY(2px)'"
                    style="transition: transform 0.3s ease"
                    class="pointer-events-none inline-block h-6 w-6 rounded-full bg-white shadow-md">
                </span>
            </button>
        </div>
        ...
        <div x-show="isSelf"
            ...
            class="border-t-2 border-blue-200/60 bg-white/70 px-6 py-5 dark:border-blue-700/40 dark:bg-gray-900/50 space-y-4">

            <div class="flex items-center gap-3">
                <div class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-900/40 text-sm">
                    ℹ️
                </div>
                <p class="text-sm font-bold text-blue-700 dark:text-blue-300">
                    Manage tenants & maintenance charges via the
                    <a href="{{ route('other-tenants.index') }}" class="font-extrabold underline decoration-blue-400/50 underline-offset-2 hover:text-blue-900 dark:hover:text-blue-100 transition-colors">
                        Other Tenants
                    </a>
                    module.
                </p>
            ...
            <div class="rounded-xl bg-blue-50 border-2 border-blue-100 p-4 dark:bg-blue-950/20 dark:border-blue-900/40">
                <p class="text-sm text-blue-900 dark:text-blue-300 leading-relaxed font-bold">
