...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-6">
        <div class="mb-6 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            <a href="{{ route('owner-receivables.index') }}" class="hover:text-brand-500">Owner Receivables</a>
            <span>/</span>
            <span class="text-gray-800 dark:text-white/90">Edit Recorded Receivable</span>
        </div>

        <x-common.component-card :title="'Edit Recorded Receivable — ' . $receivable->voucher_no" desc="Modify transaction details of this owner receivable record">
            <form action="{{ route('owner-receivables.update', $receivable) }}" method="POST" enctype="multipart/form-data" class="space-y-6"
                x-data="{
                    amount: '{{ old('amount', $receivable->amount) }}',
                    displayAmount: '',
                    formatAmount(val) {
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="owner_id" class="{{ $input }} {{ $errors->has('owner_id') ? 'border-red-400' : '' }}" required>
                            ...
                            @foreach($owners as $owner)
                                <option value="{{ $owner->id }}" {{ old('owner_id', $receivable->owner_id) == $owner->id ? 'selected' : '' }}>
                                    {{ $owner->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-form.date-picker 
                            ...
                            defaultDate="{{ old('date', $receivable->date ? $receivable->date->format('Y-m-d') : date('Y-m-d')) }}" 
                        />
                    ...
                    <div>
                        ...
                        <select name="payment_account_id" class="{{ $input }} {{ $errors->has('payment_account_id') ? 'border-red-400' : '' }}" required>
                            ...
                            @foreach($paymentAccounts as $account)
                                <option value="{{ $account->id }}" {{ old('payment_account_id', $receivable->payment_account_id) == $account->id ? 'selected' : '' }}>
                    ...
                    {{-- Reference --}}
                    <div>
                        <label class="{{ $label }}">Reference / Remarks</label>
                        <input type="text" name="reference" value="{{ old('reference', $receivable->reference) }}" placeholder="e.g. Capital Share, Slip 4920" 
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="notes" placeholder="Enter specific transaction details here..." rows="3"
                                  class="{{ $input }} {{ $errors->has('notes') ? 'border-red-400' : '' }}">{{ old('notes', $receivable->notes) }}</textarea>
                        @error('notes') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>

                    {{-- Attachment / Receipt --}}
                    <div class="sm:col-span-2">
                        <label class="{{ $label }}">Receipt / Attachment (Max 5MB)</label>
                        @if($receivable->receipt)
                            <div class="mb-2 flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50/50 p-2.5 dark:border-gray-800 dark:bg-white/[0.01]">
                                ...
                                <a href="{{ $receivable->receipt_url }}" target="_blank" class="text-sm font-semibold text-brand-600 hover:text-brand-700 underline">
