...
<head>
    ...
    <title>Owner Receivable Voucher - {{ $receivable->voucher_no }}</title>
    <!-- Tailwind CSS CDN -->
    ...
    <div class="max-w-3xl w-full mx-auto bg-white rounded-2xl border border-gray-200 shadow-sm p-8 sm:p-12 relative print-border my-auto">
        ...
        <div class="border-b border-gray-100 pb-8 mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6 print-border">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1 uppercase tracking-wider font-semibold">Owner Receivable Voucher</p>
            ...
            <div class="text-left md:text-right">
                <div class="inline-block rounded-xl px-4 py-2 bg-gray-50 text-gray-800 text-xs font-semibold print-border mb-3">
                    Voucher ID: {{ $receivable->voucher_no }}
                </div>
                <div class="space-y-1 text-xs">
                    <p><span class="text-gray-400">Date:</span> {{ $receivable->date->format('d M Y') }}</p>
        ...
        <div class="space-y-6 mb-8 text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray-100 pb-4 print-border">
                ...
                <div class="md:col-span-2 font-bold text-green-600 text-lg">Rs. {{ number_format($receivable->amount, 2) }}/-</div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray-100 pb-4 print-border">
                <div class="text-gray-450 font-medium">Received In Account:</div>
                <div class="md:col-span-2 text-gray-900 font-medium">
                    {{ $receivable->paymentAccount->name ?? '—' }}
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray-100 pb-4 print-border">
                <div class="text-gray-450 font-medium">Reference / Remarks:</div>
                <div class="md:col-span-2 font-medium text-gray-900">{{ $receivable->reference ?? '—' }}</div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray-100 pb-4 print-border">
                <div class="text-gray-450 font-medium">Recorded By:</div>
                <div class="md:col-span-2 text-gray-600 font-semibold">{{ $receivable->user->name ?? '—' }}</div>
            </div>
        </div>
        ...
        @if($receivable->notes)
            <div class="rounded-xl border border-gray-100 bg-gray-50/50 p-4 mb-8 text-sm print-border">
                ...
                <p class="text-gray-700 leading-relaxed">{{ $receivable->notes }}</p>
            </div>
        @endif
