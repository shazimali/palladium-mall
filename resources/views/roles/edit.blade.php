    ...
    <div class="mx-auto w-full">
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <form action="{{ route('roles.update', $role) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="display_name" id="display_name" value="{{ old('display_name', $role->display_name) }}" placeholder="e.g., Finance Manager"
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" placeholder="e.g., finance-manager"
                ...
                <div x-data="{
                    selected: {{ json_encode(array_map('intval', old('permissions', $rolePermissions))) }},
                    selectedGroups: {{ json_encode(array_map('intval', old('permission_groups', $roleMajorGroups))) }},
                    toggleAll(checked) {
                        if (checked) {
                    ...
                    @if($role->name === 'super-admin')
                        <div class="mb-6 rounded-xl border border-yellow-200 bg-yellow-50 p-4 text-yellow-800 dark:border-yellow-800/30 dark:bg-yellow-500/10 dark:text-yellow-400">
                            <p class="text-sm font-semibold">⚡ The super-admin role automatically overrides all permission checks in code. Adding or removing checkboxes here has no physical restriction.</p>
                        </div>
                    @endif
