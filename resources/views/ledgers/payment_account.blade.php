...
@section('fullViewTitle', 'Payment Account Ledger')
        ...
        <form action="{{ route('ledgers.payment-account') }}" method="GET" id="account-ledger-form"
            ...
            x-data="{
                ...
                options: [
                    ...
                    {
                        id: '{{ $account->id }}',
                        accountName: '{{ addslashes($account->name) }}',
                        type: '({{ ucfirst($account->type) }})',
                        text: '{{ addslashes($account->name) }} ({{ ucfirst($account->type) }})',
                        searchLabel: '{{ strtolower($account->name . " " . $account->type) }}'
                    },
                    @endforeach
                ],
                get filteredOptions() {
                ...
                get selectedAccountName() {
                    let selected = this.options.find(opt => opt.id == this.accountId);
                    return selected ? selected.accountName : '';
                },
                get selectedType() {
                    let selected = this.options.find(opt => opt.id == this.accountId);
                ...
                selectOption(opt) {
                    this.accountId = opt.id;
                    this.open = false;
                    this.search = '';
                    this.highlightedIndex = -1;
                    this.$nextTick(() => {
                        document.getElementById('account-ledger-form').submit();
                    });
                },
                moveHighlight(dir) {
                ...
                <div class="flex-1 min-w-[260px] relative" :class="open ? 'relative z-[99999]' : 'relative'" @click.away="open = false; highlightedIndex = -1">
                    ...
                    <button type="button" @click="open = !open; if(open) { $nextTick(() => $refs.searchInput.focus()) }"
                        ...
                        <template x-if="accountId">
                            <span class="flex items-center gap-2 truncate">
                                <span x-text="selectedAccountName" class="font-extrabold text-brand-600 dark:text-brand-400"></span>
                                <span x-text="selectedType" class="text-gray-500 font-semibold truncate"></span>
                            </span>
                        </template>
                        <template x-if="!accountId">
                            <span class="text-gray-400 dark:text-gray-500">Choose an Account</span>
                        </template>
                    ...
                    <div x-show="open" x-transition x-cloak
                        ...
                        <div class="p-3 border-b border-gray-100 dark:border-gray-800">
                            <div class="relative">
                                <input type="text" x-ref="searchInput" x-model="search" placeholder="Type account name or type..."
                        ...
                        <div class="max-h-64 overflow-y-auto p-2">
                            ...
                            <template x-for="(opt, index) in filteredOptions" :key="opt.id">
                                <button type="button" @click="selectOption(opt)"
                                    ...
                                    :class="accountId == opt.id ? 'bg-brand-600 text-white font-black' : (highlightedIndex === index ? 'bg-brand-50 text-brand-900 dark:bg-brand-950/40 dark:text-brand-400' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5')">
                                    <span class="flex items-center gap-2 flex-1 min-w-0">
                                        <span x-text="opt.accountName" class="font-bold"></span>
                                        <span x-text="opt.type" class="font-medium opacity-80 truncate"></span>
                                    </span>
                                    <span x-show="accountId == opt.id" class="text-sm">✔️</span>
                                </button>
                            </template>

                            <div x-show="filteredOptions.length === 0" class="px-4 py-6 text-center text-sm font-semibold text-gray-500">
                                No matching Payment Account found
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <table class="w-full text-xs sm:text-[13.5px] text-left text-gray-900 dark:text-gray-100">
                    <thead class="sticky top-0 z-10 text-xs font-black uppercase tracking-wider bg-brand-600 text-white dark:bg-brand-700 border-b-2 border-gray-200 dark:border-gray-700">
                        <tr>
                            ...
                            <th class="px-3.5 py-2.5 text-right text-white">Debit (Inflow)</th>
                            <th class="px-3.5 py-2.5 text-right text-white">Credit (Outflow)</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800 text-gray-900 dark:text-gray-100 font-semibold text-xs sm:text-[13px]">
                        @forelse($ledgerData['entries'] as $entry)
                            <tr class="{{ !empty($entry['is_opening']) ? 'bg-amber-50 dark:bg-amber-950/40 font-black' : 'hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors' }}">
                                <td class="px-3.5 py-2 text-xs sm:text-[13px] font-mono font-bold whitespace-nowrap">
                                    {{ $entry['date']->format('d M Y') }}
                                </td>
                                <td class="px-3.5 py-2 text-xs sm:text-[13px] font-bold whitespace-nowrap">
                                    @if(!empty($entry['unit_number']))
                                        <span class="unit-badge px-2 py-0.5 text-[11px] font-black rounded-md bg-brand-50 text-brand-700 dark:bg-brand-950/30 dark:text-brand-400 border border-brand-200/60 dark:border-brand-800/40">
                                ...
                                <td class="px-3.5 py-2 text-xs">
                                    @php
                                        $typeBadge = match($entry['type']) {
                                            'Receipt' => 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800',
                                            'Payout'  => 'bg-blue-50 text-blue-700 dark:bg-blue-950/20 dark:text-blue-400 border border-blue-200 dark:border-blue-800',
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-black {{ $typeBadge }}">
                                        {{ $entry['type'] }}
                                    </span>
                                </td>
                                ...
                                <td class="px-3.5 py-2 text-right font-black text-gray-900 dark:text-white font-mono text-xs sm:text-[13.5px] whitespace-nowrap">
                    ...
                    @if(count($ledgerData['entries']) > 0)
                        ...
                        <tfoot class="bg-gray-100 dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white font-black text-xs sm:text-sm">
                            <tr>
                                ...
                                <td class="px-3.5 py-2.5 text-right font-mono font-black text-gray-900 dark:text-white whitespace-nowrap">
