...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Owner Withdrawal" />

    <div class="mx-auto max-w-3xl">
        <x-common.component-card title="Edit Withdrawal Details" desc="Modify an existing profit-share withdrawal record">
            
            <form action="{{ route('withdrawals.update', $withdrawal) }}" method="POST" class="space-y-6"
                ...
                x-data="{ 
                    ...
                    displayAmount: '',
                    originalAccountId: '{{ $withdrawal->payment_account_id }}',
                    originalAmount: '{{ $withdrawal->amount }}',
                    ownerId: '{{ old('owner_id', $withdrawal->owner_id) }}',
                    originalOwnerId: '{{ $withdrawal->owner_id }}',
                    ownerPendingBalance: null,
                    ownerName: '',
                    handleSubmit(event) {
                        if (this.selectedBalance !== null && this.selectedBalance !== '' && this.amount !== '') {
                            let amt = parseFloat(this.amount);
                            let bal = this.adjustedAccountBalance;
                            if (amt > bal) {
                                Swal.fire({
                                    title: 'Insufficient Balance',
                                    text: 'The selected Payment Account does not have sufficient balance. Available: Rs. ' + bal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}),
                                    icon: 'error',
                        ...
                        if (this.ownerId !== '' && this.ownerPendingBalance !== null && this.amount !== '') {
                            let amt = parseFloat(this.amount);
                            let bal = this.adjustedOwnerBalance;
                            if (amt > bal) {
                                Swal.fire({
                    ...
                    fetchOwnerBalance(val) {
                        ...
                        fetch('{{ route('ajax.owner-pending-balance') }}?owner_id=' + val)
                            .then(r => r.json())
                            .then(d => {
                                let balance = parseFloat(d.pending_balance);
                                if (val == this.originalOwnerId) {
                                    balance += parseFloat(this.originalAmount);
                                }
                                this.ownerPendingBalance = balance;
                                this.ownerName = d.owner_name;
                            });
                    },
                    get adjustedOwnerBalance() {
                        return this.ownerPendingBalance !== null ? parseFloat(this.ownerPendingBalance) : 0;
                    },
                    get adjustedAccountBalance() {
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="owner_id" class="{{ $input }} {{ $errors->has('owner_id') ? 'border-red-400' : '' }}" required
                            ...
                            @foreach($owners as $owner)
                                <option value="{{ $owner->id }}" {{ old('owner_id', $withdrawal->owner_id) == $owner->id ? 'selected' : '' }}>
                                    {{ $owner->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <template x-if="ownerPendingBalance !== null">
                            <div class="mt-2 rounded-lg border p-2.5 text-xs font-semibold flex justify-between items-center"
                                ...
                                <span :class="ownerPendingBalance > 0 ? 'text-orange-700 dark:text-orange-400' : 'text-green-700 dark:text-green-400'">
                                    Available Dues (incl. this withdrawal):
                                </span>
                    ...
                    <div>
                        ...
                        <x-form.date-picker 
                            ...
                            defaultDate="{{ old('date', $withdrawal->date->format('Y-m-d')) }}" 
                        />
                    ...
                    <div>
                        ...
                        <select name="payment_account_id" class="{{ $input }} {{ $errors->has('payment_account_id') ? 'border-red-400' : '' }}" required
                            ...
                            @foreach($paymentAccounts as $account)
                                ...
                                <option value="{{ $account->id }}" data-balance="{{ $bal }}" data-name="{{ $account->name }}"
                                    {{ old('payment_account_id', $withdrawal->payment_account_id) == $account->id ? 'selected' : '' }}>
                        ...
                        <template x-if="selectedBalance !== null">
                            <div class="mt-2 rounded-lg border p-2.5 text-xs font-semibold flex justify-between items-center bg-gray-50 border-gray-200 dark:bg-gray-800/40 dark:border-gray-700">
                                <span class="text-gray-600 dark:text-gray-400" x-text="selectedAccountName + ' Available Balance (incl. this):'"></span>
                                <span class="font-bold text-sm text-gray-800 dark:text-white"
                                      x-text="'Rs. ' + Number(adjustedAccountBalance).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})">
                                </span>
                            </div>
                        </template>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="reference" value="{{ old('reference', $withdrawal->reference) }}" placeholder="e.g. Chq-88219"
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" placeholder="Enter transaction details..."
                        class="{{ $input }} {{ $errors->has('notes') ? 'border-red-400' : '' }}">{{ old('notes', $withdrawal->notes) }}</textarea>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-gray-150 pt-5 dark:border-gray-800">
                    ...
                    <button type="submit"
                        class="rounded-lg bg-brand-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-brand-600 transition-colors">
                        Update Withdrawal
                    </button>
                </div>
            </form>
