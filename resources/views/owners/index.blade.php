...
@section('content')
    <x-common.page-breadcrumb pageTitle="Managing Owners" />
    ...
    {{-- Warning banner for total percentage sum --}}
    @if(abs($totalShare - 100.00) > 0.01)
        <div class="mb-4 flex items-center gap-3 rounded-xl border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm text-yellow-700 dark:border-yellow-800/40 dark:bg-yellow-950/10 dark:text-yellow-400">
            ...
            <div>
                <span class="font-bold">Partnership Share Warning:</span> The sum of all managing owners' partnership shares is <span class="font-bold">{{ number_format($totalShare, 2) }}%</span>. It should equal 100.00% for correct Profit & Loss distribution. Please adjust the percentages accordingly.
            </div>
        </div>
    @endif

    <x-common.component-card title="All Managing Owners / Partners" desc="Manage Palladium Mall managing partners and partnership shares">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex flex-wrap gap-2">
                <span
                    ...
                    Total Owners: {{ $owners->total() }}
                </span>
                <span
                    class="inline-flex items-center rounded-lg bg-brand-50 px-3 py-1 text-xs font-medium text-brand-700 dark:bg-brand-950/20 dark:text-brand-400">
                    Total Share Allocated: {{ number_format($totalShare, 2) }}%
                </span>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                @if(auth()->user()->hasPermission('owners.create') || auth()->user()->isSuperAdmin())
        ...
        <div
            ...
            <form action="{{ route('owners.index') }}" method="GET" class="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <div class="relative flex-1 max-w-md">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search owners..."
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($owners as $index => $owner)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-4 py-3 text-gray-400">{{ $owners->firstItem() + $index }}</td>
                            <td class="px-4 py-3 font-semibold text-gray-800 dark:text-white/90">
                                {{ $owner->name }}
                            </td>
                            <td class="px-4 py-3">{{ $owner->phone ?? '—' }}</td>
                            <td class="px-4 py-3">{{ $owner->email ?? '—' }}</td>
                            <td class="px-4 py-3 font-semibold text-brand-600 dark:text-brand-400">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    {{-- Edit --}}
                                    @if(auth()->user()->hasPermission('owners.edit') || auth()->user()->isSuperAdmin())
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-gray-400 dark:text-gray-600">
                                ...
                                No managing owners found. <a href="{{ route('owners.create') }}" class="text-brand-500 hover:underline">Add one.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
