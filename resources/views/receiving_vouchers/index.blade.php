...
@section('content')
    ...
    <div
        x-data="{ showTable: {{ (request()->anyFilled(['search', 'unit_id', 'payment_account_id', 'date_from', 'date_to']) || request()->has('show_search')) ? 'true' : 'false' }} }">
        <x-common.component-card title="" desc="">

            {{-- Top bar --}}
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-center">
                <div class="flex flex-wrap items-center gap-3">
                    {{-- 1. New Receiving Voucher Button --}}
                    ...
                    {{-- 2. Search Button --}}
                    @if(auth()->user()->hasPermission('receiving_vouchers.search') || auth()->user()->hasPermission('receiving_vouchers.view') || auth()->user()->isSuperAdmin())
                ...
                <div
                    ...
                    <div class="flex items-center justify-between mb-4 pb-2 border-b border-gray-100 dark:border-gray-800">
                        ...
                        <div class="flex items-center gap-2">
                            @if(auth()->user()->hasPermission('receiving_vouchers.print_list') || auth()->user()->hasPermission('receiving_vouchers.print') || auth()->user()->isSuperAdmin())
                                <a href="{{ route('receiving-vouchers.print-list', request()->query()) }}" target="_blank"
                            ...
                            @endif
                            @if(request()->anyFilled(['search', 'payment_account_id', 'unit_id', 'date_from', 'date_to']))
                                <a href="{{ route('receiving-vouchers.index', ['show_search' => 1]) }}"
                    ...
                    <form x-ref="filterForm" action="{{ route('receiving-vouchers.index') }}" method="GET"
                        ...
                        <div x-data="{
                            ...
                            <div tabindex="0"
                                ...
                                <svg class="h-4 w-4 text-gray-500 transition-transform duration-200 flex-shrink-0"
                            ...
                            <div x-show="open" x-cloak
                                ...
                                <div class="px-3 pb-2 pt-1 border-b border-gray-100 dark:border-gray-700">
                                    <input x-ref="unitSearchInput" x-model="search"
                                ...
                                <ul class="max-h-60 overflow-y-auto mt-1">
                                    <template x-if="filteredUnits.length === 0">
                                        <li class="px-4 py-2 text-xs text-gray-500 dark:text-gray-400">No matches found.
                                        </li>
                                    </template>
                        ...
                        <div x-data="{
                            ...
                            <div x-show="open" x-cloak
                                class="absolute left-0 z-50 mt-1 w-full rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 py-2">
                                <div class="px-3 pb-2 pt-1 border-b border-gray-100 dark:border-gray-700">
                                    <input x-ref="accountSearchInput" x-model="search"
                                        @keydown.arrow-down.prevent="moveHighlight(1)"
                                        @keydown.arrow-up.prevent="moveHighlight(-1)"
                                        @keydown.enter.prevent="selectHighlighted()"
                                        @keydown.escape.prevent="open = false; highlightedIndex = -1" type="text"
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @if(auth()->user()->hasPermission('receiving_vouchers.delete') || auth()->user()->isSuperAdmin())
                                                <form action="{{ route('receiving-vouchers.destroy', $voucher) }}" method="POST"
                                                    x-data
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-12 text-center text-gray-400 dark:text-gray-600">
                                        ...
                                        No receiving vouchers found. <a href="{{ route('receiving-vouchers.create') }}"
                                            class="text-brand-500 hover:underline">Record one now.</a>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
