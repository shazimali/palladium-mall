...
<head>
    ...
    <title>Receipt Voucher - {{ $voucher->voucher_no }}</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-gray-50 text-gray-800 antialiased min-h-screen flex flex-col justify-between py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-3xl w-full mx-auto bg-white rounded-2xl border border-gray-200 shadow-sm p-8 sm:p-12 relative print-border my-auto">
        ...
        <div class="border-b border-gray-100 pb-8 mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6 print-border">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1 uppercase tracking-wider font-semibold">Management Office Receipt Voucher</p>
        ...
        <div class="space-y-6 mb-8 text-sm">
            @php
                ...
                if ($voucher->received_from_type === 'tenant') {
                    ...
                    $fromName = $voucher->other_name;
                }
            @endphp

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray-100 pb-4 print-border">
                <div class="text-gray-450 font-medium">Received From:</div>
                <div class="md:col-span-2 font-bold text-gray-900 text-base">{{ $fromName }} 
                    <span class="text-xs font-semibold text-gray-450">({{ ucfirst($voucher->received_from_type) }})</span>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray-100 pb-4 print-border">
                <div class="text-gray-450 font-medium">Amount Received:</div>
                <div class="md:col-span-2 font-bold text-green-600 text-lg">Rs. {{ number_format($voucher->amount, 2) }}/-</div>
            </div>
        ...
        {{-- Tenant Bill Allocations Table --}}
        @if($voucher->received_from_type === 'tenant' && $voucher->payments->isNotEmpty())
            <div class="mb-8">
                <h3 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-4">Allocated Dues / Billings</h3>
                ...
                <table class="w-full text-left text-sm print-border">
                    <thead>
                        <tr class="border-b border-gray-200 text-gray-400 text-xs uppercase font-semibold">
                            <th class="py-2.5 font-semibold">Flat/Shop</th>
                            <th class="py-2.5 font-semibold">Billing Month</th>
                            <th class="py-2.5 font-semibold">Charge Type</th>
                            <th class="py-2.5 text-right font-semibold">Allocated Amount</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @foreach($voucher->payments as $payment)
                            <tr>
                                <td class="py-3 font-semibold text-gray-950">{{ $payment->unit->unit_number ?? '—' }}</td>
                                <td class="py-3 text-xs text-gray-500">{{ $payment->month ? $payment->month->format('M Y') : '—' }}</td>
                                <td class="py-3 text-xs text-gray-500">{{ $payment->type_label }}</td>
    ...
    <div class="text-center text-xs text-gray-400 mt-8 no-print">
        <p>This is a computer-generated receipt voucher copy. Printed on {{ now()->format('d M Y H:i:s') }}</p>
    </div>
