...
@section('fullViewTitle', 'Landlord Ledger')
...
@section('content')
    ...
    <x-common.component-card title="" desc="">
        ...
        @if($ledgerData)
            ...
            <!-- Ledger Table -->
            <div data-ledger-table class="overflow-auto max-h-[70vh] rounded-2xl border-2 border-gray-200 dark:border-gray-800 shadow-md">
                <table class="w-full text-base sm:text-lg text-left text-gray-900 dark:text-gray-100">
                    <thead class="sticky top-0 z-10 text-sm sm:text-base font-black uppercase tracking-wider bg-brand-600 text-white dark:bg-brand-700 border-b-2 border-gray-200 dark:border-gray-700">
                        <tr>
                                ...
                                <td class="px-5 py-4 text-base sm:text-lg font-mono font-black">
                                    @if(!empty($entry['model']))
                                        @if($entry['model'] instanceof \App\Models\ReceivingVoucher)
                                            <a href="{{ route('receiving-vouchers.show', $entry['model']->id) }}" class="text-brand-600 hover:underline font-mono font-black dark:text-brand-400">
                                                {{ $entry['voucher_no'] }}
                                            </a>
                                        @elseif($entry['model'] instanceof \App\Models\GeneralReceivingVoucher)
                                            <a href="{{ route('general-receiving-vouchers.show', $entry['model']->id) }}" class="text-brand-600 hover:underline font-mono font-black dark:text-brand-400">
                                                {{ $entry['voucher_no'] }}
                                            </a>
                                        @elseif($entry['model'] instanceof \App\Models\PaymentVoucher)
                                            <a href="{{ route('payment-vouchers.show', $entry['model']->id) }}" class="text-brand-600 hover:underline font-mono font-black dark:text-brand-400">
                                                {{ $entry['voucher_no'] }}
                                            </a>
                                        @else
