...
<head>
    ...
    <title>Print Payment Vouchers Report - PALLADIUM MALL</title>
    <!-- Tailwind CSS CDN -->
    ...
    <div class="max-w-6xl w-full mx-auto printable-container rounded-3xl bg-white p-6 sm:p-10 border border-gray-300 shadow-xl text-gray-900 font-sans">
        ...
        <div class="mb-6 rounded-2xl bg-gray-50 border border-gray-300 p-4 font-sans text-xs">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-gray-800 font-semibold">
                ...
                <div>
                    <span class="text-gray-500 block text-[10px] uppercase font-bold">Payee Type:</span>
                    <span class="font-bold text-gray-900">
                        {{ !empty($filters['paid_to_type']) ? ucfirst($filters['paid_to_type']) : 'All Types' }}
                    </span>
                </div>
                <div>
                    <span class="text-gray-500 block text-[10px] uppercase font-bold">Paid From Account:</span>
                    <span class="font-bold text-gray-900">{{ $selectedAccount ?? 'All Accounts' }}</span>
                </div>
                <div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-200 mb-6 text-sm font-bold">
            <div>
                ...
                <span class="text-gray-900 font-black ml-1">{{ number_format(count($vouchers)) }}</span>
            </div>
            <div>
                <span class="text-gray-500">Total Paid Out:</span>
                <span class="text-blue-700 font-black ml-1 text-base">Rs. {{ number_format($totalAmount, 2) }}</span>
            </div>
            <div>
                    ...
                    @forelse($vouchers as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-2.5 px-3 font-bold text-gray-900">
                                @if($item->paid_to_type === 'owner')
                                    {{ $item->owner->name ?? '—' }} <span class="text-[10px] text-gray-500 font-normal">(Owner)</span>
                                @elseif($item->paid_to_type === 'tenant')
                                    {{ $item->tenant->name ?? '—' }} <span class="text-[10px] text-gray-500 font-normal">(Tenant)</span>
                                @elseif($item->paid_to_type === 'landlord')
                                    {{ $item->landlord->name ?? '—' }} <span class="text-[10px] text-gray-500 font-normal">(Landlord)</span>
                                @elseif($item->paid_to_type === 'party')
                                    {{ $item->party->name ?? '—' }} <span class="text-[10px] text-gray-500 font-normal">(Party)</span>
                                @elseif($item->paid_to_type === 'account')
                                    {{ $item->toPaymentAccount->name ?? '—' }} <span class="text-[10px] text-gray-500 font-normal">(Account Transfer)</span>
                                @else
                                    {{ $item->other_name ?? '—' }}
                                @endif
                            </td>
                            ...
                            <td class="py-2.5 px-3 text-right font-black text-blue-700 font-mono text-sm whitespace-nowrap">
                ...
                @if(count($vouchers) > 0)
                    <tfoot>
                        <tr class="bg-gray-100 border-t-2 border-gray-300 font-black text-xs text-gray-900 uppercase">
                            ...
                            <td class="py-3 px-3 text-right text-blue-700 font-mono text-base whitespace-nowrap">
