...
<body onload="window.print()">
    ...
    <div class="grid">
        ...
        <div class="item"><span class="label">Father/Husband Name:</span><span
                class="value">{{ $tenant->father_name ?? 'N/A' }}</span></div>
        <div class="item"><span class="label">CNIC Number:</span><span class="value">{{ $tenant->cnic }}</span></div>
        <div class="item"><span class="label">Date of Birth:</span><span
                class="value">{{ optional($tenant->date_of_birth)->format('d M Y') ?? 'N/A' }}</span></div>
        <div class="item"><span class="label">Gender:</span><span
                class="value">{{ ucfirst($tenant->gender ?? 'N/A') }}</span></div>
        <div class="item"><span class="label">Marital Status:</span><span
                class="value">{{ ucfirst($tenant->marital_status ?? 'N/A') }}</span></div>
    </div>

    <div class="section-title">Contact Information</div>
    <div class="grid">
        <div class="item"><span class="label">Phone:</span><span class="value">{{ $tenant->phone }}</span></div>
        <div class="item"><span class="label">WhatsApp:</span><span
                class="value">{{ $tenant->whatsapp_number ?? 'N/A' }}</span></div>
        <div class="item"><span class="label">Email:</span><span class="value">{{ $tenant->email ?? 'N/A' }}</span>
        </div>
        <div class="item"><span class="label">Occupation:</span><span
                class="value">{{ $tenant->occupation ?? 'N/A' }}</span></div>
        <div class="item" style="grid-column: span 2;"><span class="label">Permanent Address:</span><span
                class="value">{{ $tenant->address }}</span></div>
    </div>

    <div class="section-title">Additional Details</div>
    <div class="grid">
        <div class="item"><span class="label">Monthly Income:</span><span
                class="value">{{ $tenant->monthly_income ? number_format($tenant->monthly_income) . ' PKR' : 'N/A' }}</span>
        </div>
        <div class="item"><span class="label">Tenancy Type:</span><span
                class="value">{{ ucfirst($tenant->tenancy_type ?? 'N/A') }}</span></div>
        <div class="item"><span class="label">Adults in Family:</span><span
                class="value">{{ $tenant->adults_count ?? 1 }}</span></div>
        <div class="item"><span class="label">Children in Family:</span><span
                class="value">{{ $tenant->children_count ?? 0 }}</span></div>
    </div>

    <div class="section-title">Assigned Flat / Shop Details</div>
    <div class="grid">
        <div class="item"><span class="label">Flat/Shop Number:</span><span
                class="value"><strong>{{ $tenant->unit ? $tenant->unit->unit_number : 'Not Assigned' }}</strong></span>
        </div>
        <div class="item"><span class="label">Type:</span><span
                class="value">{{ $tenant->unit ? ucfirst($tenant->unit->type) : 'N/A' }}</span></div>
        <div class="item"><span class="label">Floor:</span><span
                class="value">{{ $tenant->unit?->floor?->name ?? 'N/A' }}</span></div>
        <div class="item"><span class="label">Block:</span><span
                class="value">{{ $tenant->unit?->block?->name ?? 'N/A' }}</span></div>
        @if($tenant->unit?->area)
            <div class="item"><span class="label">Area / Zone:</span><span
                    class="value">{{ $tenant->unit->area->name }}</span></div>
        @endif
        @if($tenant->unit?->area_sqft)
            <div class="item"><span class="label">Size (sqft):</span><span
                    class="value">{{ number_format($tenant->unit->area_sqft, 2) }}</span></div>
        @endif
    </div>
    ...
    @if(($partners ?? $tenant->partners) && ($partners ?? $tenant->partners)->isNotEmpty())
        @foreach(($partners ?? $tenant->partners) as $index => $partner)
            ...
            <div class="grid">
                ...
                <div class="item"><span class="label">Father/Husband Name:</span><span
                        class="value">{{ $partner->father_name ?? 'N/A' }}</span></div>
                <div class="item"><span class="label">CNIC Number:</span><span class="value">{{ $partner->cnic }}</span></div>
                <div class="item"><span class="label">Gender:</span><span
                        class="value">{{ ucfirst($partner->gender ?? 'N/A') }}</span></div>
                <div class="item"><span class="label">Marital Status:</span><span
                        class="value">{{ ucfirst($partner->marital_status ?? 'N/A') }}</span></div>
                <div class="item"><span class="label">Phone:</span><span class="value">{{ $partner->phone }}</span></div>
                <div class="item"><span class="label">WhatsApp:</span><span
                        class="value">{{ $partner->whatsapp_number ?? 'N/A' }}</span></div>
                <div class="item"><span class="label">Email:</span><span class="value">{{ $partner->email ?? 'N/A' }}</span>
                </div>
                <div class="item"><span class="label">Occupation:</span><span
                        class="value">{{ $partner->occupation ?? 'N/A' }}</span></div>
                <div class="item"><span class="label">Monthly Income:</span><span
                        class="value">{{ $partner->monthly_income ? number_format($partner->monthly_income) . ' PKR' : 'N/A' }}</span>
                </div>
                <div class="item" style="grid-column: span 2;"><span class="label">Permanent Address:</span><span
                        class="value">{{ $partner->address ?? 'N/A' }}</span></div>
            </div>
    ...
    <div style="position: relative; text-align: center; margin-bottom: 20px; border-bottom: 2px solid #111; padding-bottom: 12px; min-height: 65px;">
        ...
        @endif
        <h2 style="margin: 0 0 4px; font-size: 24px; font-weight: 900; letter-spacing: 1.5px; text-transform: uppercase; color: #111;">PALLADIUM MALL</h2>
        <h1 style="margin: 0 0 8px; font-size: 18px; font-weight: 800; text-transform: uppercase; color: #333;">Guarantor Information</h1>
        <p style="margin: 6px 0 0; font-size: 15px; font-weight: bold; color: #111;">
            Tenant: <span style="font-size: 17px; font-weight: 900; color: #000;">{{ $tenant->name }}</span> | Unit: <span style="font-size: 17px; font-weight: 900; color: #000;">{{ $tenant->unit ? $tenant->unit->unit_number . ($tenant->unit->floor ? ' (' . $tenant->unit->floor->name . ')' : '') . ($tenant->unit->block ? ' - ' . $tenant->unit->block->name : '') : 'N/A' }}</span>
        </p>
    </div>
    ...
    @forelse($guarantors as $index => $g)
        <div class="guarantor-card">
            <div style="font-weight: bold; margin-bottom: 10px; text-transform: uppercase; font-size: 12px; color: #555;">
                Guarantor #{{ $index + 1 }}</div>
            <div style="display: flex; gap: 20px; align-items: start;">
                @if($g->photo)
                    <img src="{{ $g->photo_url }}"
                        style="width: 100px; height: 100px; border-radius: 4px; object-fit: cover; border: 1px solid #ccc; flex-shrink: 0;"
                        alt="Guarantor Photo">
                @else
                    <div
                        style="width: 100px; height: 100px; border-radius: 4px; border: 1px dashed #ccc; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; color: #888; font-size: 11px; flex-shrink: 0;">
                        <span>⚠️ Photo<br>Needed</span>
                    </div>
                @endif
                <div style="flex-grow: 1;">
                    <div class="grid">
                        <div class="item"><span class="label">Guarantor Name:</span><span
                                class="value">{{ $g->name }}</span></div>
                        <div class="item"><span class="label">CNIC Number:</span><span class="value">{{ $g->cnic }}</span>
                        </div>
                        <div class="item"><span class="label">Relation to Tenant:</span><span
                                class="value">{{ ucfirst($g->relation) }}</span></div>
                        <div class="item"><span class="label">Phone:</span><span class="value">{{ $g->phone }}</span></div>
                        <div class="item"><span class="label">Occupation:</span><span
                                class="value">{{ $g->occupation ?? 'N/A' }}</span></div>
                        @if($g->shop_name)
                            <div class="item"><span class="label">Shop Name:</span><span
                                    class="value">{{ $g->shop_name }}</span></div>
                        @endif
                        <div class="item" style="grid-column: span 2;"><span class="label">Address:</span><span
                                class="value">{{ $g->address }}</span></div>
                    </div>
                </div>
            </div>
    ...
    <div style="position: relative; text-align: center; margin-bottom: 20px; border-bottom: 2px solid #111; padding-bottom: 12px; min-height: 65px;">
        ...
        @endif
        <h2 style="margin: 0 0 4px; font-size: 24px; font-weight: 900; letter-spacing: 1.5px; text-transform: uppercase; color: #111;">PALLADIUM MALL</h2>
        <h1 style="margin: 0 0 8px; font-size: 18px; font-weight: 800; text-transform: uppercase; color: #333;">Tenancy Agreement Terms</h1>
        <p style="margin: 6px 0 0; font-size: 15px; font-weight: bold; color: #111;">
            Tenant: <span style="font-size: 17px; font-weight: 900; color: #000;">{{ $tenant->name }}</span> | Unit: <span style="font-size: 17px; font-weight: 900; color: #000;">{{ $tenant->unit ? $tenant->unit->unit_number . ($tenant->unit->floor ? ' (' . $tenant->unit->floor->name . ')' : '') . ($tenant->unit->block ? ' - ' . $tenant->unit->block->name : '') : 'N/A' }}</span>
        </p>
    </div>
    ...
    <div class="grid">
        ...
        <div class="item"><span class="label">Assigned Unit:</span><span
                class="value"><strong>{{ $tenant->unit ? $tenant->unit->unit_number : 'N/A' }}</strong></span></div>
        <div class="item"><span class="label">Unit Type:</span><span
                class="value">{{ $tenant->unit ? ucfirst($tenant->unit->type) : 'N/A' }}</span></div>
        <div class="item"><span class="label">Floor:</span><span
                class="value">{{ $tenant->unit?->floor?->name ?? 'N/A' }}</span></div>
        <div class="item"><span class="label">Block:</span><span
    ...
    @if($agreement)
        ...
        <div class="grid">
            <div class="item"><span class="label">Start Date:</span><span
                    class="value">{{ optional($agreement->start_date)->format('d M Y') }}</span></div>
            <div class="item"><span class="label">End Date:</span><span
                    class="value">{{ optional($agreement->end_date)->format('d M Y') }}</span></div>
            <div class="item"><span class="label">Monthly Rent:</span><span
                    class="value">{{ number_format($agreement->monthly_rent) }} PKR</span></div>
            <div class="item"><span class="label">Security Deposit:</span><span
                    class="value">{{ number_format($agreement->security_deposit) }} PKR</span></div>
            <div class="item"><span class="label">Maintenance Charge:</span><span
                    class="value">{{ $agreement->maintenance_charge ? number_format($agreement->maintenance_charge) . ' PKR' : 'N/A' }}</span>
            </div>
            <div class="item"><span class="label">Payment Due Day:</span><span
                    class="value">{{ $agreement->payment_due_day }}th of each month</span></div>
            <div class="item"><span class="label">Grace Period:</span><span
                    class="value">{{ $agreement->grace_period_days ?? 0 }} days</span></div>
            <div class="item"><span class="label">Fine Per Day:</span><span
                    class="value">{{ $agreement->fine_per_day ? number_format($agreement->fine_per_day) . ' PKR' : '0 PKR' }}</span>
            </div>
            <div class="item"><span class="label">Notice Period:</span><span
                    class="value">{{ $agreement->notice_period_months ?? 0 }} month(s)</span></div>
        </div>
    ...
    <div class="intro-text">
        Dear Client, please ensure the following documents are submitted to the company to complete your onboarding
        process. All required documents are checked below with their current verification status.
    </div>
            ...
            <tbody>
                @foreach($items as $item)
                    @php
                        ...
                        $fileUrl = $checklist?->getFileUrlForField($item['field']);
                        $filePath = $checklist?->getFilePathForField($item['field']);
                        $fileAttrVal = $filePath ? $checklist?->{$filePath} : null;
    ...
    @if($checklist?->notes)
        ...
        <p
            style="font-size: 13px; color: #555; background: #fafafa; border: 1px solid #eee; padding: 10px; border-radius: 4px;">
            {{ $checklist->notes }}
        </p>
    @endif
    ...
    <div style="position: relative; text-align: center; margin-bottom: 20px; border-bottom: 2px solid #111; padding-bottom: 12px; min-height: 65px;">
        ...
        @endif
        <h2 style="margin: 0 0 4px; font-size: 24px; font-weight: 900; letter-spacing: 1.5px; text-transform: uppercase; color: #111;">PALLADIUM MALL</h2>
        <h1 style="margin: 0 0 8px; font-size: 18px; font-weight: 800; text-transform: uppercase; color: #333;">Move-in Inspection Checklist</h1>
        <p style="margin: 6px 0 0; font-size: 15px; font-weight: bold; color: #111;">
            Tenant: <span style="font-size: 17px; font-weight: 900; color: #000;">{{ $tenant->name }}</span> | Unit: <span style="font-size: 17px; font-weight: 900; color: #000;">{{ $tenant->unit ? $tenant->unit->unit_number . ($tenant->unit->floor ? ' (' . $tenant->unit->floor->name . ')' : '') . ($tenant->unit->block ? ' - ' . $tenant->unit->block->name : '') : 'N/A' }}</span>
        </p>
    </div>
    ...
    <div class="meta-bar">
        <div><span>Inspection Date:</span> <strong>{{ optional($moveInChecklist?->checklist_date)->format('d M Y') ?? now()->format('d M Y') }}</strong></div>
        <div><span>Inspector:</span> <strong>{{ $moveInChecklist?->inspection_member ?? 'N/A' }}</strong></div>
        <div><span>Condition:</span> <strong>{{ $moveInChecklist?->flat_condition ? ucfirst($moveInChecklist->flat_condition) : 'N/A' }}</strong></div>
    </div>
        ...
        <div class="checklist-grid">
            @foreach($items as $field => $itemLabel)
                <div class="checklist-item">
                    <span
                        class="check-box">{!! ($moveInChecklist && $moveInChecklist->{$field}) ? '&#9745;' : '&#9744;' !!}</span>
                    <span>{{ $itemLabel }}</span>
                </div>
            @endforeach
        </div>
    ...
    <div class="signature-area" style="display: flex; justify-content: space-between; gap: 10px; margin-top: 20px;">
        <div class="sig-box" style="flex: 1; border-top: 1px solid #111; text-align: center; padding-top: 4px; font-size: 10px; font-weight: bold; text-transform: uppercase;">Tenant</div>
        <div class="sig-box" style="flex: 1; border-top: 1px solid #111; text-align: center; padding-top: 4px; font-size: 10px; font-weight: bold; text-transform: uppercase;">Dealer</div>
        <div class="sig-box" style="flex: 1; border-top: 1px solid #111; text-align: center; padding-top: 4px; font-size: 10px; font-weight: bold; text-transform: uppercase;">ASM</div>
        <div class="sig-box" style="flex: 1; border-top: 1px solid #111; text-align: center; padding-top: 4px; font-size: 10px; font-weight: bold; text-transform: uppercase;">Accountant</div>
        <div class="sig-box" style="flex: 1; border-top: 1px solid #111; text-align: center; padding-top: 4px; font-size: 10px; font-weight: bold; text-transform: uppercase;">MD</div>
    </div>
