        ...
        @foreach($steps as $n => $label)
            ...
            <div class="flex flex-col items-center {{ $n < count($steps) ? 'flex-1' : '' }}">
                <div class="flex items-center w-full">
                    ...
                    @if($n > 1)
                        <div
                            class="flex-1 h-0.5 transition-colors duration-300 {{ ($n <= $currentStep) ? 'bg-brand-500' : 'bg-gray-200 dark:bg-gray-700' }}">
                        </div>
                    @endif
                    ...
                    <div class="relative flex-shrink-0">
                        @if($isActive)
                            ...
                            @endif
                        @elseif($isCompleted && $tenantId)
                            <a href="{{ route('tenants.showStep', [$tenantId, $n]) }}" title="Go back to {{ $label }}"
                ...
                <span
                    class="mt-2 text-xs font-bold {{ $isActive ? 'text-green-600 dark:text-green-400' : ($isCompleted ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500') }}">
                    {{ $label }}
                </span>
            </div>
        @endforeach
