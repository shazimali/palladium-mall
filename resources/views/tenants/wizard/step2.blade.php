    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <form method="POST" action="{{ route('tenants.saveStep', [$tenant, 2]) }}" class="px-6 py-6 space-y-6">
            ...
            <div class="rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-gray-800 dark:bg-white/[0.02]">
                <h4 class="mb-4 text-sm font-semibold uppercase tracking-wide text-gray-700 dark:text-gray-300">Guarantor (Mandatory)</h4>
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="guarantor_name" value="{{ old('guarantor_name', $g->name ?? '') }}"
                               placeholder="Full name" class="{{ $input }} {{ $errors->has('guarantor_name') ? 'border-red-400' : '' }}">
                        @error('guarantor_name') <p class="{{ $error }}">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="{{ $label }}">CNIC <span class="text-red-500">*</span></label>
                        <input type="text" name="guarantor_cnic" value="{{ old('guarantor_cnic', $g->cnic ?? '') }}"
                               placeholder="35201-1234567-1" maxlength="15" class="{{ $input }} {{ $errors->has('guarantor_cnic') ? 'border-red-400' : '' }}">
                    ...
                    <div>
                        ...
                        <input type="text" name="guarantor_phone" value="{{ old('guarantor_phone', $g->phone ?? '') }}"
                               placeholder="0300-1234567" class="{{ $input }} {{ $errors->has('guarantor_phone') ? 'border-red-400' : '' }}">
                        @error('guarantor_phone') <p class="{{ $error }}">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="{{ $label }}">Occupation</label>
                        <input type="text" name="guarantor_occupation" value="{{ old('guarantor_occupation', $g->occupation ?? '') }}"
                               placeholder="e.g. Businessman" class="{{ $input }}">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="{{ $label }}">Address <span class="text-red-500">*</span></label>
                        <input type="text" name="guarantor_address" value="{{ old('guarantor_address', $g->address ?? '') }}"
                               placeholder="Full address" class="{{ $input }} {{ $errors->has('guarantor_address') ? 'border-red-400' : '' }}">
            ...
            {{-- ── Emergency Contacts ────────────────────────────────────── --}}
            <div class="rounded-xl border border-gray-100 bg-gray-50 p-5 dark:border-gray-800 dark:bg-white/[0.02]" x-data="{ contacts: {{ json_encode(
                old('contacts', $emergencyContacts->count() > 0
                ...
                <div class="mb-4 flex items-center justify-between">
                    <h4 class="text-sm font-semibold uppercase tracking-wide text-gray-700 dark:text-gray-300">Emergency Contacts (min. 2)</h4>
                    ...
                    <template x-for="(contact, index) in contacts" :key="index">
                        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800/50">
                            <div class="mb-3 flex items-center justify-between">
                                <span class="text-xs font-semibold text-gray-500 uppercase" x-text="'Contact ' + (index + 1)"></span>
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    <label class="{{ $label }}">Relation <span class="text-red-500">*</span></label>
                                    <select :name="'contacts[' + index + '][relation]'" x-model="contact.relation" class="{{ $select }}">
                                        <option value="">Select relation</option>
                                ...
                                <div>
                                    ...
                                    <input type="text" :name="'contacts[' + index + '][address]'" x-model="contact.address"
                                           placeholder="Optional" class="{{ $input }}">
                                </div>
                            </div>
                        </div>
                    </template>
