...
<head>
    ...
    <title>{{ $billHeading ?? 'Maintenance & Utilities Bill' }} -
        {{ $payment->tenant->name ?? $payment->otherTenant->name ?? 'N/A' }}</title>
    <!-- Tailwind CSS CDN -->
...
<body
    class="bg-gray-100 text-gray-900 antialiased min-h-screen flex flex-col justify-between py-10 px-4 sm:px-6 lg:px-8 text-[13px]">
    ...
    <div class="print-sheet mx-auto bg-white border-2 border-gray-800 shadow-sm relative my-auto">
        ...
        <div class="absolute -top-14 right-0 flex items-center gap-3 no-print">
            <button onclick="window.print()"
                class="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700 transition-colors shadow-sm">
            ...
            <button onclick="window.close()"
                class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
                Close Window
            </button>
        </div>

        @php
            $totalBilled = $groupedPayments->sum('amount');
            $totalPaid = $groupedPayments->sum('amount_paid');
            $balanceDue = $groupedPayments->sum(fn($r) => $r->balanceDue());
        ...
        <div class="grid grid-cols-5 border-b-2 border-gray-800">
            <div class="col-span-3 p-3 border-r-2 border-gray-800">
                ...
                <div class="grid grid-cols-3 gap-px bg-gray-800 mt-2 text-[10px] border border-gray-800">
                    ...
                    <div class="bg-white px-1.5 py-1">{{ $payment->month ? $payment->month->format('M-Y') : 'N/A' }}
                    </div>
                    <div class="bg-white px-1.5 py-1">{{ now()->format('d M Y') }}</div>
                    <div class="bg-white px-1.5 py-1">
                        {{ $payment->due_date ? $payment->due_date->format('d M Y') : 'N/A' }}</div>
                </div>
            </div>
            <div class="col-span-2 p-3 flex flex-col justify-center items-end text-right">
                <p class="text-[9px] uppercase tracking-widest text-gray-400 font-semibold">
                    {{ $billEyebrow ?? 'Consolidated' }}</p>
                <h2 class="text-sm font-extrabold tracking-tight leading-tight">
                    {{ $billHeading ?? 'MAINTENANCE & UTILITIES BILL' }}</h2>
            </div>
        </div>
        ...
        <div class="grid grid-cols-5 border-b-2 border-gray-800">
            <div class="col-span-3 p-3 border-r-2 border-gray-800">
                ...
                <p class="font-bold text-[12px]">{{ $billTenant->name ?? 'N/A' }}</p>
                @if($billTenant && $billTenant->cnic)
                    <p class="text-[10px] text-gray-600 mt-0.5">CNIC # {{ $billTenant->cnic }}</p>
                @endif
                <p class="text-[10px] text-gray-600">Unit No: {{ $unitLabel }} @if($payment->unit)&middot;
                {{ ucfirst($payment->unit->type) }}@endif</p>
                @if($billTenant && $billTenant->address)
                    <p class="text-[10px] text-gray-600">{{ $billTenant->address }}</p>
                @endif
            </div>
        ...
        <!-- Body: services + totals (left) / instructions + history (right) -->
        <div class="grid grid-cols-5">
            <div class="col-span-3 border-r-2 border-gray-800 flex flex-col">
                <table class="w-full text-left text-[11px]">
                    <thead>
                        <tr class="border-b-2 border-gray-800 bg-gray-100 text-[10px] uppercase">
                            <th class="px-2.5 py-1 font-semibold">Service</th>
                        ...
                        @foreach($groupedPayments as $record)
                            <tr>
                                <td class="px-2.5 py-1">
                                    <p class="font-medium">{{ $record->type_label }}</p>
                                    @if(in_array($record->type, ['electricity', 'water', 'gas']))
                                        <p class="text-[9px] text-gray-500">{{ number_format($record->units_consumed, 2) }}
                                            units @ Rs. {{ number_format($record->rate_per_unit, 2) }}</p>
                                    @endif
                                </td>
                                <td class="px-2.5 py-1 text-right font-medium">{{ number_format($record->amount, 2) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                <table class="w-full text-[11px] border-t-2 border-gray-800">
                    <tbody class="divide-y divide-gray-200">
                        <tr>
                            ...
                            <td class="px-2.5 py-1 text-right font-semibold">{{ number_format($totalBilled, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="px-2.5 py-1 text-gray-600">Amount Paid</td>
                            <td class="px-2.5 py-1 text-right text-green-700">{{ number_format($totalPaid, 2) }}</td>
                        </tr>
                        <tr class="bg-gray-100">
                            <td class="px-2.5 py-1 font-extrabold">Balance Due</td>
                            <td class="px-2.5 py-1 text-right font-extrabold">{{ number_format($balanceDue, 2) }}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="px-2.5 py-1.5 border-t border-gray-300 text-[9px] text-gray-500 leading-relaxed">
                    ...
                    <p>1. The above charges are in lieu of general maintenance, cleaning and security services for the
                        unit.</p>
                    <p>2. Please deposit this bill at the management office before the due date to avoid surcharges.</p>
                </div>
            </div>
            ...
            <div class="col-span-2 flex flex-col">
                <div class="p-2.5 border-b-2 border-gray-800 bg-gray-50" dir="rtl">
                    ...
                    <ol class="font-urdu list-decimal list-inside text-[11px] text-gray-700 space-y-0 text-right">
                        <li>براہ کرم مقررہ تاریخ سے پہلے بل جمع کروائیں تاکہ اضافی جرمانہ عائد نہ ہو۔</li>
                        <li>بل صرف پیلاڈیم مال مینجمنٹ آفس میں جمع کروائیں۔</li>
                        <li>یہ بل ادائیگی کے ثبوت کے طور پر محفوظ رکھیں۔</li>
                        <li>بل میں کسی بھی غلطی کی صورت میں 7 دن کے اندر مینجمنٹ آفس سے رابطہ کریں۔</li>
                    </ol>
                </div>
                        ...
                        <table class="w-full text-[10px]">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($billingHistory as $row)
                                    <tr>
                                        <td class="py-0.5">{{ \Illuminate\Support\Carbon::parse($row->month)->format('M-Y') }}
                                        </td>
                ...
                <div class="p-2.5 text-[10px] text-gray-600 space-y-0">
                    <h4 class="text-[9px] font-bold uppercase tracking-wider text-gray-500 mb-0.5">Contact / Complaints
                    </h4>
    ...
    <div class="text-center text-[10px] text-gray-400 mt-4 no-print">
        <p>This is a computer-generated consolidated bill and does not require a physical stamp. Printed on
