...
@isset($summary)
    <div id="ajax-paginator-meta" class="hidden" data-total="{{ $payments->total() }}"
        ...
        data-rent-unpaid="Rs. {{ number_format($summary['rent_unpaid'] ?? 0) }}" @php
         ...
         @endphp
        data-services-due="Rs. {{ number_format($servDue) }}" data-services-paid="Rs. {{ number_format($servPaid) }}"
...
@else
    <div id="ajax-paginator-meta" class="hidden" data-total="{{ $payments->total() }}"></div>
@endisset

<div class="overflow-hidden border border-gray-200 rounded-xl dark:border-gray-800">
    <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
        <thead
            class="text-xs sm:text-sm uppercase font-black bg-brand-500 text-white dark:bg-brand-600 dark:text-white">
            <tr>
                ...
                <th class="px-4 py-3.5 text-white text-center">Print</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            @forelse($groupedPayments as $key => $group)
                ...
                @endphp
                <tr
                    class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors {{ request('search') ? 'bg-amber-500/[0.03] dark:bg-amber-500/[0.02] border-l-2 border-l-amber-500/70' : '' }}">
                    ...
                    <td class="px-4 py-3 align-middle max-w-xl">
                        <div class="flex flex-col gap-2">
                            @foreach($group as $payment)
                                <div
                                    class="flex flex-wrap items-center justify-between gap-3 p-2 rounded-xl border border-gray-100 bg-gray-50/50 dark:border-gray-800/40 dark:bg-white/[0.01]">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span
                                            class="inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-bold {{ $payment->type_badge_class }}">
                                        ...
                                        <span class="text-[10px] text-gray-500">
                                            Paid: <span
                                                class="{{ $payment->amount_paid > 0 ? 'text-green-600 font-medium' : 'text-gray-400' }}">Rs.
                                                {{ number_format($payment->amount_paid) }}</span>
                                        </span>
                                        <span
                                            class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $payment->status_badge_class }}">
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        @if(auth()->user()->hasPermission('payments.delete') || auth()->user()->isSuperAdmin())
                                            <form action="{{ route('payments.destroy', $payment) }}" method="POST" x-data
                                                @submit.prevent="Swal.fire({ title: 'Delete payment record?', text: 'Are you sure you want to delete this payment record?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Yes, delete it!' }).then((result) => { if (result.isConfirmed) $el.submit(); })">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="inline-flex items-center rounded-lg p-1 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                                                    title="Delete">
                                                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <td class="px-4 py-3 font-semibold text-gray-800 dark:text-white/90 align-middle">Rs.
                    ...
                    <td class="px-4 py-3 align-middle text-center">
                        <a href="{{ route('payments.print-bill', $firstPayment) }}" target="_blank"
                            class="inline-flex items-center justify-center rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-brand-600 dark:hover:bg-white/10 transition-colors"
                            title="Print Consolidated Bill">
