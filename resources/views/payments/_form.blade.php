    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3"
        ...
        <div class="relative"
            ...
            <div x-ref="cbTrigger"
                 ...
                 class="w-full cursor-pointer flex justify-between items-center rounded-2xl border-2 bg-white px-5 py-3.5 text-lg font-bold dark:bg-gray-900 {{ $errors->has('unit_id') ? 'border-red-400' : 'border-gray-300 dark:border-gray-700' }}">
                <span x-text="cbSelectedLabel || 'Select Flat/Shop…'"
                      :class="cbSelectedLabel ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-600'"></span>
            ...
            <div x-show="cbOpen"
                ...
                <ul class="max-h-72 overflow-y-auto mt-1 p-1 space-y-1" x-ref="optionsList">
                    ...
                    <template x-for="(u, index) in filteredUnits" :key="u.id">
                        <li @click="pickUnit(u)"
                            ...
                            :data-index="index"
                            :class="cbActiveIndex === index ? 'bg-brand-600 text-white font-black' : 'text-gray-900 dark:text-white font-bold hover:bg-gray-100 dark:hover:bg-white/5'"
                            ...
                            <span x-text="u.unit_number" class="font-black text-lg sm:text-xl"></span>
        ...
        {{-- Tenant (auto-filled) --}}
        <div>
            <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Tenant</label>
            <div class="w-full rounded-2xl border-2 border-gray-200 bg-gray-100 px-5 py-3.5 text-lg font-bold dark:border-gray-700 dark:bg-gray-800">
                <span x-text="tenantName || 'Auto-filled when flat/shop is selected'"
                      :class="tenantName ? 'text-gray-900 dark:text-white font-black' : 'text-gray-400 dark:text-gray-500'">
                </span>
            </div>
        </div>
        ...
        <div>
            <label class="mb-2 block text-xs sm:text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">Landlord</label>
            <div class="w-full rounded-2xl border-2 border-gray-200 bg-gray-100 px-5 py-3.5 text-lg font-bold dark:border-gray-700 dark:bg-gray-800">
                <span x-text="landlordName || 'Auto-filled when flat/shop is selected'"
                      :class="landlordName ? 'text-gray-900 dark:text-white font-black' : 'text-gray-400 dark:text-gray-500'">
                </span>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div>
            ...
            <input type="text" id="month" name="month"
                ...
                class="w-full rounded-2xl border-2 border-gray-300 bg-white px-5 py-3.5 text-lg font-bold text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white {{ $errors->has('month') ? 'border-red-400' : '' }}">
            @error('month')
                <p class="mt-1.5 text-sm font-semibold text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" id="amount" name="amount" value="{{ old('amount', $payment->amount ?? '') }}" min="0"
                ...
                class="w-full rounded-2xl border-2 border-gray-300 bg-white px-5 py-3.5 text-lg font-bold text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white {{ $errors->has('amount') ? 'border-red-400' : '' }}">
            <p class="mt-1 text-xs font-semibold text-gray-400">Auto-filled from active agreement. Override if needed.</p>
            @error('amount')
                <p class="mt-1.5 text-sm font-semibold text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" id="due_date" name="due_date"
                ...
                class="w-full rounded-2xl border-2 border-gray-300 bg-white px-5 py-3.5 text-lg font-bold text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white {{ $errors->has('due_date') ? 'border-red-400' : '' }}">
            @error('due_date')
                <p class="mt-1.5 text-sm font-semibold text-red-500">{{ $message }}</p>
            @enderror
        </div>
