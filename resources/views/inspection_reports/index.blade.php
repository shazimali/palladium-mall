                ...
                @can("{$permPrefix}.create")
                    ...
                    @else
                        <button type="button" disabled
                                title="Daily reports can only be generated between {{ $reportType->time_window_display }}"
                                class="inline-flex items-center gap-1.5 rounded-lg bg-gray-200 px-4 py-2.5 text-sm font-bold text-gray-500 cursor-not-allowed opacity-75 dark:bg-gray-800 dark:text-gray-400">
                            🔒 Window Closed ({{ $reportType->time_window_display }})
                        </button>
                    @endif
                @endcan
            </div>
                        ...
                        @forelse($reports as $report)
                            <tr class="hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="px-4 py-3 text-xs">
                                    @php
                                        $satCount = $report->items->where('admin_rating', 'good')->count();
                                        $unsatCount = $report->items->where('admin_rating', 'bad')->count();
