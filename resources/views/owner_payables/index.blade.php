...
@section('content')
    <x-common.page-breadcrumb pageTitle="Owner Payable Vouchers" />
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Outflow (Filtered)</p>
    ...
    <x-common.component-card title="Owner Payable Ledger Book" desc="View and manage all cash payouts, reimbursement expenses, or salaries paid to managing owners">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div class="flex flex-wrap gap-2">
                <span class="inline-flex items-center rounded-lg bg-gray-100 px-3 py-1 text-xs font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                    Showing Page: {{ $payables->currentPage() }} of {{ $payables->lastPage() }}
                </span>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                <a href="{{ route('owner-payables.create') }}"
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <form action="{{ route('owner-payables.index') }}" method="GET" class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-5 items-end">
                @php
            ...
            <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                <thead class="border-b border-gray-100 bg-gray-50/50 text-xs uppercase text-gray-500 dark:border-gray-800 dark:bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-4 py-3">Owner (Recipient)</th>
                    ...
                    @forelse($payables as $payable)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                            ...
                            <td class="px-4 py-3.5 whitespace-nowrap">
                                {{ $payable->date ? $payable->date->format('d M Y') : '—' }}
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                @if($payable->paymentAccount)
                                    <div class="text-gray-800 dark:text-white/80 font-medium">{{ $payable->paymentAccount->name }}</div>
                                    <div class="text-[10px] text-gray-400 uppercase tracking-wide">{{ $payable->paymentAccount->type }}</div>
                                @else
                                    <span class="text-gray-400">—</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 max-w-[16rem] truncate" title="{{ $payable->notes }}">
                                @if($payable->reference)
                                    <span class="font-medium text-gray-700 dark:text-gray-300">Ref: {{ $payable->reference }}</span>
                                @endif
                                @if($payable->notes)
                                    <div class="text-xs text-gray-400 truncate">{{ $payable->notes }}</div>
                                @endif
                                @if(!$payable->reference && !$payable->notes)
                                    <span class="text-gray-400">—</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 text-right font-bold text-red-600 dark:text-red-400">
                                Rs. {{ number_format($payable->amount, 2) }}
                            </td>
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('owner-payables.show', $payable) }}"
                                    ...
                                    <a href="{{ route('owner-payables.edit', $payable) }}"
                                    ...
                                    @if(auth()->user()->isSuperAdmin())
                                        <form action="{{ route('owner-payables.destroy', $payable) }}" method="POST"
                                            @submit.prevent="confirmAction($el, 'Are you sure you want to delete Owner Payable Voucher {{ $payable->voucher_no }} for Rs. {{ number_format($payable->amount, 2) }}?', 'Delete Voucher?', 'Yes, Delete')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-12 text-center text-gray-400 dark:text-gray-600">
                                No owner payables matching filters.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
