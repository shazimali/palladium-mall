...
@section('content')
    <x-common.page-breadcrumb pageTitle="Voucher Details — {{ $payable->voucher_no }}" />

    <x-common.component-card title="Owner Payable Voucher Details" desc="Voucher Reference #{{ $payable->voucher_no }}">
        
        <div class="mb-6 flex justify-end gap-3 no-print">
            <a href="{{ route('owner-payables.print', $payable) }}" target="_blank"
            ...
            @if(auth()->user()->isSuperAdmin())
                <form action="{{ route('owner-payables.destroy', $payable) }}" method="POST" x-data
                    @submit.prevent="confirmAction($el, 'Are you sure you want to cancel and delete this Owner Payable Voucher of Rs. {{ number_format($payable->amount) }}? This action is irreversible.', 'Cancel Voucher?', 'Yes, Cancel')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-6">
            @foreach([
                ['Voucher Number',      $payable->voucher_no],
                ['Voucher Date',        $payable->date->format('d M Y')],
                ['Managing Owner',      $payable->owner->name ?? '—'],
                ['Voucher Amount',      'Rs. ' . number_format($payable->amount, 2)],
                ['Payment Account',     $payable->paymentAccount ? $payable->paymentAccount->name : '—'],
                ['Payment Method',      $payable->paymentAccount ? strtoupper($payable->paymentAccount->type) : '—'],
                ['Reference / Cheque',  $payable->reference ?? '—'],
                ['Recorded By',         $payable->user->name ?? '—'],
            ] as [$label, $value])
        ...
        @if($payable->notes)
            <div class="rounded-lg bg-gray-50 px-4 py-3 dark:bg-white/[0.03] mb-6">
                ...
                <p class="mt-0.5 text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{{ $payable->notes }}</p>
            </div>
        @endif
        ...
        @if($payable->receipt)
            <div class="rounded-lg bg-gray-50 px-4 py-3 dark:bg-white/[0.03] mb-6">
                ...
                <a href="{{ $payable->receipt_url }}" target="_blank"
