...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-6">
        <div class="mb-6 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <span>/</span>
            <span class="text-gray-800 dark:text-white/90">Edit Recorded Payable</span>
        </div>

        <x-common.component-card :title="'Edit Recorded Payable — ' . $payable->voucher_no" desc="Modify transaction details of this owner payable record">
            <form action="{{ route('owner-payables.update', $payable) }}" method="POST" enctype="multipart/form-data" class="space-y-6"
                x-data="{
                    amount: '{{ old('amount', $payable->amount) }}',
                    displayAmount: '',
                    formatAmount(val) {
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="owner_id" class="{{ $input }} {{ $errors->has('owner_id') ? 'border-red-400' : '' }}" required>
                            ...
                            @foreach($owners as $owner)
                                <option value="{{ $owner->id }}" {{ old('owner_id', $payable->owner_id) == $owner->id ? 'selected' : '' }}>
                                    {{ $owner->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-form.date-picker 
                            ...
                            defaultDate="{{ old('date', $payable->date ? $payable->date->format('Y-m-d') : date('Y-m-d')) }}" 
                        />
                    ...
                    <div>
                        ...
                        <select name="payment_account_id" class="{{ $input }} {{ $errors->has('payment_account_id') ? 'border-red-400' : '' }}" required>
                            ...
                            @foreach($paymentAccounts as $account)
                                <option value="{{ $account->id }}" {{ old('payment_account_id', $payable->payment_account_id) == $account->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="reference" value="{{ old('reference', $payable->reference) }}" placeholder="e.g. Salary June, Cheque 4893" 
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="notes" placeholder="Enter specific transaction details here..." rows="3"
                                  class="{{ $input }} {{ $errors->has('notes') ? 'border-red-400' : '' }}">{{ old('notes', $payable->notes) }}</textarea>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        @if($payable->receipt)
                            <div class="mb-2 flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50/50 p-2.5 dark:border-gray-800 dark:bg-white/[0.01]">
                                ...
                                <a href="{{ $payable->receipt_url }}" target="_blank" class="text-sm font-semibold text-brand-600 hover:text-brand-700 underline">
