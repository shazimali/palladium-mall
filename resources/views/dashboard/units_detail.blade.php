    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <span
                        class="inline-flex items-center gap-2 rounded-full bg-blue-50 dark:bg-blue-950/40 px-3.5 py-1 text-sm font-extrabold text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 shadow-xs">
                        <span>Total: {{ $counts['total'] }}</span>
                        <span class="text-blue-300 dark:text-blue-700">•</span>
                        <span>Flats: {{ $counts['flats'] }}</span>
                        <span class="text-blue-300 dark:text-blue-700">•</span>
                        <span>Shops: {{ $counts['shops'] }}</span>
                        @if($counts['offices'] > 0)
                            <span class="text-blue-300 dark:text-blue-700">•</span>
                            <span>Offices: {{ $counts['offices'] }}</span>
                        @endif
                    </span>
                </div>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Floor and block-wise distribution list of all flats, shops, and offices.
                </p>
            </div>
        ...
        @if(empty($status))
            <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                {{-- Total --}}
                <a href="{{ route('dashboard.units-detail', ['type' => $type]) }}"
                ...
                {{-- Rented (Green) --}}
                <a href="{{ route('dashboard.units-detail', ['type' => $type, 'status' => 'rented']) }}"
                    class="group relative block overflow-hidden rounded-2xl p-5 text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl flex flex-col justify-between"
                ...
                {{-- Vacant (Red) --}}
                <a href="{{ route('dashboard.units-detail', ['type' => $type, 'status' => 'vacant']) }}"
                    class="group relative block overflow-hidden rounded-2xl p-5 text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl flex flex-col justify-between"
                                ...
                                <div class="rounded-xl p-4 text-white shadow-md flex flex-col justify-between transition-all duration-200 hover:shadow-xl hover:scale-[1.03]"
                                    ...
                                    <div class="mt-3 pt-2 border-t border-white/20 flex items-center justify-between">
                                        <span class="text-xs font-black uppercase tracking-wider text-white">
                                            {{ $isEffectiveRented ? 'Rented' : ($isEffectiveVacant ? 'Vacant' : ucfirst($unit->status)) }}
                                        </span>
                                    </div>
                                </div>
                            @endforeach
