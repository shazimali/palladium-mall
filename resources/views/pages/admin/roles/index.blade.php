                    ...
                    @forelse($roles as $role)
                        <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
                            <td class="px-5 py-4 sm:px-6">
                                <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $role->display_name }}</span>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <code class="rounded bg-gray-100 px-2 py-1 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-400">{{ $role->name }}</code>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <span class="text-theme-sm text-gray-500 dark:text-gray-400">{{ $role->description ?? 'No description provided.' }}</span>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <span class="text-theme-xs inline-block rounded-full bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-400 px-2.5 py-0.5 font-medium">
                            ...
                            <td class="px-5 py-4 sm:px-6">
                                <span class="text-theme-sm text-gray-700 dark:text-white/90 font-medium">{{ $role->users_count }}</span>
                            </td>
                            <td class="px-5 py-4 text-right sm:px-6">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.roles.edit', $role) }}" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-400">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                                    @if($role->name !== 'super-admin')
                                        <form action="{{ route('admin.roles.destroy', $role) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this role?')" class="inline">
                                            @csrf
                                            @method('DELETE')
