...
@section('content')
    ...
    <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
                @if(request()->anyFilled(['search', 'role']))
                    <a href="{{ route('admin.users.index') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-white/5">
                        Clear Filters
                    </a>
                @endif
                <button type="submit" class="rounded-lg bg-gray-800 px-4 py-2 text-sm font-medium text-white hover:bg-gray-900 dark:bg-white/10 dark:hover:bg-white/20">
                    Apply Filter
                </button>
                <a href="{{ route('admin.users.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">
                    Create User
                </a>
            </div>
        </form>
                        ...
                        <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
                            ...
                            <td class="px-5 py-4 sm:px-6">
                                <form action="{{ route('admin.users.toggle-status', $user) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" 
                                        ...
                                        class="text-theme-xs inline-block rounded-full px-2.5 py-0.5 font-semibold transition cursor-pointer {{ $user->is_active ? 'bg-green-50 text-green-700 dark:bg-green-500/15 dark:text-green-500 hover:bg-green-100' : 'bg-red-50 text-red-700 dark:bg-red-500/15 dark:text-red-500 hover:bg-red-100' }} {{ $user->id === auth()->id() ? 'opacity-50 cursor-not-allowed hover:bg-green-50 hover:bg-red-50' : '' }}">
                            ...
                            <td class="px-5 py-4 text-right sm:px-6">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.users.edit', $user) }}" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-400">
