...
@section('content')
    <x-common.page-breadcrumb pageTitle="Gate Passes (Outflows)" />
    ...
    <x-common.component-card title="Gate Pass Logs" desc="View list of material checkout permits issued to maintenance staff.">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
            <div class="flex flex-wrap gap-2">
                <span class="inline-flex items-center rounded-lg bg-gray-100 px-3 py-1 text-xs font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                    Total Permits: {{ $gatePasses->total() }}
                </span>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @if(auth()->user()->hasPermission('gatepasses.manage') || auth()->user()->isSuperAdmin())
                    <a href="{{ route('gate-passes.create') }}"
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <form action="{{ route('gate-passes.index') }}" method="GET" class="flex flex-col gap-4 sm:flex-row sm:items-center">
                <!-- Search Input -->
                <div class="relative flex-1 max-w-md">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by GP #, worker name, or purpose..."
                ...
                <div>
                    <select name="status" onchange="this.form.submit()"
                        class="h-10 rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
            ...
            <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
                <thead class="text-xs uppercase bg-gray-50 text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                    <tr>
                        ...
                        <th class="px-5 py-3.5">Gate Pass #</th>
                        <th class="px-5 py-3.5">Issued To</th>
                        <th class="px-5 py-3.5">Purpose / Work Area</th>
                    ...
                    @forelse($gatePasses as $gp)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-5 py-3.5 text-xs font-mono">
                                {{ $gp->date->format('d M Y') }}
                            </td>
                            <td class="px-5 py-3.5 font-bold font-mono text-gray-850 dark:text-white">
                                <a href="{{ route('gate-passes.show', $gp) }}" class="text-brand-500 hover:underline">
                                    {{ $gp->gatepass_no }}
                                </a>
                            </td>
                            <td class="px-5 py-3.5 font-semibold text-gray-850 dark:text-white">
                                {{ $gp->issued_to }}
                            </td>
                            <td class="px-5 py-3.5 text-xs">
                                <div>{{ $gp->purpose }}</div>
                                @if($gp->unit)
                                    <span class="inline-block bg-gray-100 text-gray-650 px-2 py-0.5 rounded text-[10px] mt-1 dark:bg-gray-800 dark:text-gray-300">
                                        Flat/Shop: {{ $gp->unit->unit_number }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-5 py-3.5 font-mono text-xs">
                                {{ $gp->items->count() }} items
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    {{-- Print --}}
                                    <a href="{{ route('gate-passes.print', $gp) }}" target="_blank"
                                        class="inline-flex items-center rounded-lg p-1.5 text-gray-600 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                                    ...
                                    {{-- Cancel --}}
                                    @if($gp->status === 'Issued' && (auth()->user()->hasPermission('gatepasses.manage') || auth()->user()->isSuperAdmin()))
                                        <form action="{{ route('gate-passes.cancel', $gp) }}" method="POST" x-data
                                            @submit.prevent="if(confirm('Are you sure you want to CANCEL this Gate Pass? Items stock will be returned.')) $el.submit()">
                                            @csrf
                                            <button type="submit"
                                                class="inline-flex items-center rounded-lg p-1.5 text-amber-500 hover:bg-amber-50 hover:text-amber-700 dark:hover:bg-amber-900/20 transition-colors"
                                    ...
                                    {{-- Delete --}}
                                    @if(auth()->user()->hasPermission('gatepasses.manage') || auth()->user()->isSuperAdmin())
                                        <form action="{{ route('gate-passes.destroy', $gp) }}" method="POST" x-data
                                            @submit.prevent="if(confirm('Are you sure you want to delete this Gate Pass? This will rollback item quantities.')) $el.submit()">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-gray-400 dark:text-gray-600">
                                ...
                                No Gate Passes issued yet. <a href="{{ route('gate-passes.create') }}" class="text-brand-500 hover:underline">Issue a Gate Pass.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
